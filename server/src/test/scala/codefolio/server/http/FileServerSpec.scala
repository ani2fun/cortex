...
object FileServerSpec extends ZIOSpecDefault:
    ...
    suite("serve")(
      ...
      test("404s a `..` traversal that escapes the root") {
        ZIO.scoped {
          for
            root    <- tempRoot
            outside <- ZIO.attempt(Files.createTempFile("codefolio-fileserver-outside-", ".txt"))
      ...
      test("404s a symlink that escapes the root") {
        ZIO.scoped {
          for
            root    <- tempRoot
            outside <- ZIO.attempt(Files.createTempFile("codefolio-fileserver-symlink-", ".txt"))
            _       <- ZIO.attempt(Files.writeString(outside, "secret"))
            _       <- ZIO.addFinalizer(ZIO.attempt(Files.deleteIfExists(outside)).orDie)
  ...
  private val tempRoot: ZIO[Scope, Throwable, Path] =
    ZIO.acquireRelease(ZIO.attempt(Files.createTempDirectory("codefolio-fileserver-"))) { root =>
