...
object ChapterAssetRewrite:
  ...
  def rewrite(
    ...
    val afterHtml = htmlAttrPattern.replaceAllIn(
      ...
      m =>
        val url = m.group(2)
        if isRewritable(url) then Matcher.quoteReplacement(s"""${m.group(1)}"${toAbsolute(url)}"""")
    ...
    mdLinkPattern.replaceAllIn(
      ...
      m =>
        val url = m.group(3)
        if isRewritable(url) then
          Matcher.quoteReplacement(s"""${m.group(1)}[${m.group(2)}](${toAbsolute(url)}${m.group(4)})""")
        else Matcher.quoteReplacement(m.matched)
    )
