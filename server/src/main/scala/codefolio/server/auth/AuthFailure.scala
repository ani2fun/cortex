 ...
 *
 * Joins the `HandlerFailure` union in [[codefolio.server.http.ApiErrors]] alongside the per-pipeline failure
...
object AuthFailure:
  ...
  /**
   * The server has `codefolio.auth.enabled = false`. Returned by [[Auth.verify]] (not
