...
import codefolio.server.config.{AuthConfig, RateLimitBucket, RateLimitConfig, RedisConfig}
import codefolio.shared.api.Endpoints.Quota
 ...
 *
 * When auth is disabled (`codefolio.auth.enabled = false`, the `bin/dev` default) the limiter is a no-op:
