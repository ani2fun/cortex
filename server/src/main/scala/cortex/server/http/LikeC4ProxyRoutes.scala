 ...
 * The browser hits cortex at `kakde.eu/c4/...`; cortex forwards those requests to the upstream LikeC4
 * deployment. There is no public Ingress on the LikeC4 deployment.
 *
 * Posture mirrors piston: ClusterIP only + NetworkPolicy in apps-prod permitting cortex → likec4 on port
 * 8080.
 *
 * v1 demo notes:
