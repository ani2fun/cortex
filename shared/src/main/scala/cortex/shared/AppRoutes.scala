...
object AppRoutes:
   ...
   * for each of these segments so a hard reload re-enters the SPA. `Cortex` is the legacy `/cortex/...` prefix:
   * the server serves index.html and the client router redirects `/cortex/x` → `/x` (see ADR-0009).
   */
