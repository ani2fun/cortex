...
Inside an f-string ([Tutorial 4](/cortex/languages/python/first-steps/strings-the-basics)), a colon after the value introduces a **format spec** controlling width, alignment, precision, and representation.
...
`join` can't concatenate an `int`, so it raises. Convert first: `"-".join(str(n) for n in [1, 2, 3])` (a generator expression — [Tutorial 14](/cortex/languages/python/working-with-data/comprehensions)) gives `"1-2-3"`.
...
Slicing ([Tutorial 12](/cortex/languages/python/working-with-data/sequences)) gives strings a set of one-line idioms.
