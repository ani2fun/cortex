...
This is the deep pass of [Functions, the Basics](/cortex/languages/python/control-flow/functions-the-basics) — it assumes you've met `def`, `return`, and parameters, and pushes into the object nature of functions, scope, closures, decorators, and purity. It builds directly on [The Object Model](/cortex/languages/python/how-python-works/the-object-model) (functions are objects with attributes) and on the generator expressions from [Comprehensions](/cortex/languages/python/working-with-data/comprehensions). Every output below was produced by running the code.
...
**Intuition.**
*Mechanism.* "Returning multiple values" is a convenient fiction — `return a, b` builds a single tuple `(a, b)` (the comma makes the tuple — see [Sequences](/cortex/languages/python/working-with-data/sequences)) and returns that one object. Tuple unpacking on the receiving end makes it *feel* like several values came back.
...
Functions carry metadata you can inspect at runtime — because they're objects (the central fact from [The Object Model](/cortex/languages/python/how-python-works/the-object-model)).
...
*Predict, then check.* Retype the two famous traps — the mutable default (§4) and the lambda late-binding loop (§10) — from memory, and predict each output before running. When you can explain why *both* come down to one rule — "defaults and closures are decided at definition time, bodies run at call time" — you've understood the deepest thing this chapter teaches, and you're ready for [Errors & Exceptions](/cortex/languages/python/how-python-works/errors-and-exceptions).
