...
Every `for` loop you've written ([Tutorial 8](/cortex/languages/python/control-flow/loops)) and every comprehension and generator expression ([Tutorial 14](/cortex/languages/python/working-with-data/comprehensions)) rests on one small protocol. The thesis: **`for` is syntactic sugar — under it, `iter()` produces an *iterator*, `next()` pulls one item at a time, and a `StopIteration` signal ends the loop** — and `yield` is the easy way to write your own iterators that produce values *lazily*, one at a time, without ever building a list.

This is the deep pass of [loops](/cortex/languages/python/control-flow/loops) and [generator expressions](/cortex/languages/python/working-with-data/comprehensions). Every output below was produced by running the code.
