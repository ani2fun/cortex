...
*Earned rule.* `range(n)` gives `n` values, `0` to `n − 1`. To count `1` to `n`, use `range(1, n + 1)` (next section). The cost of zero-based, stop-exclusive ranges is the constant temptation to off-by-one — but it's the same rule as string indexing ([Tutorial 4](/cortex/languages/python/first-steps/strings-the-basics)) and slicing, so it pays to internalise once.

---

## 3. Looping over a string

`for` works on any sequence, and a string is a sequence of characters ([Tutorial 4](/cortex/languages/python/first-steps/strings-the-basics)). Looping over a string gives you each **character**, in order.
...
*Earned rule.* Reach for nested loops to cover every combination (grids, pairs), but watch the multiplied cost — it's the difference between instant and frozen. The boundary: when nesting gets deep or slow, that's the signal to look for a better data structure or algorithm, a theme [Performance](/cortex/languages/python/advanced/performance-and-profiling) returns to in Tier 5.
