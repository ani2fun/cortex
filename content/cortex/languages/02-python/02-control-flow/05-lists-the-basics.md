...
Write a list with square brackets and commas. Index it exactly like a string ([Tutorial 4](/cortex/languages/python/first-steps/strings-the-basics)): positions start at `0`, `-1` is the last, `len()` counts the items. `in` tests membership.

```python run viz=array viz-root=fruits
...
The `in` operator ([Tutorial 6](/cortex/languages/python/control-flow/booleans-and-logic)) checks whether a value is an element:
...
**Intuition.**
*Mechanism.* Index assignment (`lst[i] = x`) mutates the list in place, replacing slot `i`. This is the defining difference from strings, which are **immutable** ([Tutorial 4](/cortex/languages/python/first-steps/strings-the-basics)) and forbid it.
...
```python run viz=array viz-root=nums
nums = [1, 2, 3]
nums.append(4)
nums.insert(0, 0)
nums.remove(2)
...
**Intuition.**
*Mechanism.* `append`/`insert`/`remove` mutate in place and return `None` (their job is the side effect — [Tutorial 9](/cortex/languages/python/control-flow/loop-control-and-patterns)). `pop` is the exception: it mutates *and* returns the removed item, so you can use it.
...
You loop over a list exactly like a string ([Tutorial 8](/cortex/languages/python/control-flow/loops)) — the loop variable is each **item**.
