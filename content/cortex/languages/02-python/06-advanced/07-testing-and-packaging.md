...
`assert (1 == 2, "msg")` asserts a two-element *tuple*, and a non-empty tuple is always truthy ([Tutorial 6](/cortex/languages/python/control-flow/booleans-and-logic)) — so the check is silently disabled and the next line runs. (Python prints a `SyntaxWarning: assertion is always true, perhaps remove parentheses?` to stderr — heed it.) Write `assert cond, "msg"` with **no** parentheses around the pair.
