...
This draws on [complexity](/cortex/languages/python/working-with-data/sequences) and [sets](/cortex/languages/python/working-with-data/dictionaries-and-sets). Every output below was produced by running the code; timing figures are marked illustrative because they vary run to run, but the *relationships* between them are the point.
...
*Concrete bite.* The numbers are the bite: **14,000×** from one data-structure change. Programmers routinely spend hours shaving constant factors (a faster loop, a `+=` tweak — [Tutorial 15](/cortex/languages/python/working-with-data/strings-in-depth)) while an `O(n²)` algorithm hides in plain sight. The hours-long micro-optimisation is dwarfed by a one-line `list` → `set`.
