...
This builds on every type you've met and on [the object model](/cortex/languages/python/how-python-works/the-object-model). Every runnable output below was produced by running the code; the one `mypy` example is marked, since the runner has no type checker installed.
