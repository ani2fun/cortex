...
You have used objects since [Tutorial 1](/cortex/languages/python/first-steps/what-is-python) — a `list`, a `dict`, a `str` are all objects, and you have called their methods (`.append`, `.get`, `.upper`). Now you build your own. The thesis: **a class is a blueprint that bundles *data* (attributes) with *behaviour* (methods); an instance is one concrete thing made from that blueprint; and `self` is how a method names the particular instance it was called on.** A `Dog` class describes what every dog has and can do; `Dog("Rex")` makes one specific dog.

Classes are not a new kind of value — they sit squarely on top of the [object model](/cortex/languages/python/how-python-works/the-object-model) you already know, and methods are just [functions](/cortex/languages/python/control-flow/functions-the-basics) that live on a class. Every output below was produced by running the code — including the deliberate tracebacks.
