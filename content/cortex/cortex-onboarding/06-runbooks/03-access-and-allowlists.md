...
Don't conflate the allowlist with **`COACH_HOMELAB_USERS`** — a *tutor* env var, **not** this table. It decides a signed-in learner's **coach model tier**: the operator's homelab / Ollama models plus quota exemption, versus the BYOK default (Gemini 2.5 Flash). It's managed on the tutor deployment, and a change needs a pod restart:
