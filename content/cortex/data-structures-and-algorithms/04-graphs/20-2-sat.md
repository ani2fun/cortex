...
A four-clause instance over three variables. The solver builds the implication graph, runs Tarjan's SCC, and reads off an assignment — then verifies it. The input is the number of variables on the first line and the clause list on the second. Pick a case and **Run** it.
...
n = int(input())
clauses = ast.literal_eval(input())
a = solve_2sat(n, clauses)
# print assignment with lowercase bools
a_str = "[" + ", ".join("true" if v else "false" for v in a) + "]"
print("assignment:", a_str)
def lit(l): return a[abs(l) - 1] if l > 0 else not a[abs(l) - 1]
ok = all(lit(x) or lit(y) for x, y in clauses)
print("all clauses satisfied:", "true" if ok else "false")
```
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int[][] clauses = parseIntMatrix(sc.nextLine());
        boolean[] a = solve2sat(n, clauses);
        // print assignment with lowercase bools
        StringBuilder sb = new StringBuilder("[");
        for (int i = 0; i < a.length; i++) { if (i > 0) sb.append(", "); sb.append(a[i]); }
        sb.append("]");
        System.out.println("assignment: " + sb);
        boolean ok = true;
        for (int[] c : clauses) {
            boolean lx = c[0] > 0 ? a[c[0]-1] : !a[-c[0]-1];
            boolean ly = c[1] > 0 ? a[c[1]-1] : !a[-c[1]-1];
            if (!(lx || ly)) { ok = false; break; }
        }
...
{
  "args": [
    { "id": "n", "label": "n (number of variables)", "type": "number", "placeholder": "3" },
    { "id": "clauses", "label": "clauses (signed 1-indexed literals)", "type": "int[][]", "placeholder": "[[1, 2], [-1, 3], [2, -3], [-2, -3]]" }
  ],
  "cases": [
    {
      "args": { "n": "3", "clauses": "[[1, 2], [-1, 3], [2, -3], [-2, -3]]" },
      "expected": "assignment: [false, true, false]\nall clauses satisfied: true"
    },
    {
      "args": { "n": "2", "clauses": "[[1, 2], [-1, 2], [1, -2]]" },
      "expected": "assignment: [true, true]\nall clauses satisfied: true"
    },
    {
      "args": { "n": "2", "clauses": "[[1, 2], [-1, -2]]" },
      "expected": "assignment: [true, false]\nall clauses satisfied: true"
    }
  ],
  "verifying": "run"
}
...
Both print `assignment: [false, true, false]` (x₁ = false, x₂ = true, x₃ = false) and confirm every clause holds. The assignment is deterministic: Tarjan's SCC ordering is identical in both languages given the same adjacency list, so `scc_id[x] < scc_id[¬x]` resolves the same way.
...
print("satisfiable:", "true" if sat else "false")
print("SCC of x1:", scc_id[0], " SCC of ¬x1:", scc_id[1], " same SCC:", "true" if scc_id[0] == scc_id[1] else "false")
```
...
print("true" if two_colourable(n, edges) else "false")
```
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(twoColourable(n, edges));
    }
}
```
...
{
  "args": [
    { "id": "n", "label": "n (number of nodes)", "type": "number", "placeholder": "3" },
    { "id": "edges", "label": "edges (undirected edge list)", "type": "int[][]", "placeholder": "[[0, 1], [1, 2], [2, 0]]" }
  ],
  "cases": [
    {
      "args": { "n": "3", "edges": "[[0, 1], [1, 2], [2, 0]]" },
      "expected": "false"
    },
    {
      "args": { "n": "4", "edges": "[[0, 1], [1, 2], [2, 3], [3, 0]]" },
      "expected": "true"
    },
    {
      "args": { "n": "5", "edges": "[[0, 1], [1, 2], [2, 3], [3, 4], [4, 0]]" },
      "expected": "false"
    }
  ],
  "verifying": "solution"
...
**Approach:** For each edge `(u, v)` in the undirected graph, the two endpoints must have different colours, which is the 2-SAT constraint `(c_u ∨ c_v) ∧ (¬c_u ∨ ¬c_v)`. Translate each constraint to two implications in the implication graph, then run Tarjan's SCC. If any variable `x` and its negation `¬x` land in the same SCC, the graph is not 2-colourable (has an odd cycle). Time: `O(V + E)`. Space: `O(V + E)`.
...
- 2-SAT pre-processing appears in production SAT solvers (MiniSat, Glucose). The assignment, UNSAT-with-shared-SCC, and triangle/square 2-colouring outputs above all come from the runnable blocks — re-run to verify.
