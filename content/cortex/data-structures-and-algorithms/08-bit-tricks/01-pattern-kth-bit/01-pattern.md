...
n = int(input())
k = int(input())
mask = 1 << (k - 1)           # a single 1 at position k

print("true" if (n & mask) else "false")  # check : is bit k set?
print(n | mask)               # set   : force bit k to 1
print(n & ~mask)              # unset : force bit k to 0
print(n ^ mask)               # toggle: flip bit k
```
...
public class Main {
  public static void main(String[] args) {
    ...
    int mask = 1 << (k - 1);  // a single 1 at position k

    System.out.println((n & mask) != 0);  // check : is bit k set?
    System.out.println(n | mask);          // set   : force bit k to 1
    System.out.println(n & ~mask);         // unset : force bit k to 0
    System.out.println(n ^ mask);          // toggle: flip bit k
  }
}
```
...
```testcases
{
  ...
  "cases": [
    { "args": { "n": "10", "k": "3" }, "expected": "false\n14\n10\n14" },
    { "args": { "n": "7", "k": "2" }, "expected": "true\n7\n5\n5" },
    { "args": { "n": "0", "k": "1" }, "expected": "false\n1\n0\n1" }
  ]
}
```
...
The four reusable primitives — fill in each body, then Run. The driver reads `n` and `k`, prints: check result (`true`/`false`), set result, clear result, toggle result:
...
n = int(input())
k = int(input())
print("true" if check(n, k) else "false")
print(set_bit(n, k))
print(clear(n, k))
print(toggle(n, k))
```
...
```testcases
{
  ...
  "cases": [
    { "args": { "n": "10", "k": "2" }, "expected": "true\n10\n8\n8" },
    { "args": { "n": "10", "k": "1" }, "expected": "false\n11\n10\n11" },
    { "args": { "n": "15", "k": "4" }, "expected": "true\n15\n7\n7" }
  ]
}
```
...
- The mask-plus-operator primitives are standard; both runnable blocks are verified by running.
