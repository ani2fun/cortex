...
summary: "Given two arrays arr1 and arr2 (where arr2 is a subset of arr1 and all elements are unique), return for each value in arr2 its preceding superior element in arr1 — the first strictly-greater element to its left in arr1. Return -1 for values with no preceding superior."
prereqs:
...
class Solution:
    def preceding_superior_element(self, arr_1: List[int], arr_2: List[int]) -> List[int]:
        # Your code goes here — build the previous-greater array for arr_1,
        # then answer each arr_2 query with an index-map lookup.
        return []

arr1 = ast.literal_eval(input())
arr2 = ast.literal_eval(input())
print(Solution().preceding_superior_element(arr1, arr2))
```
...
public class Main {
    static class Solution {
        public int[] precedingSuperiorElement(int[] arr1, int[] arr2) {
            // Your code goes here — build the previous-greater array for arr1,
            // then answer each arr2 query with an index-map lookup.
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(new Solution().precedingSuperiorElement(arr1, arr2)));
    }
}
```
...
```testcases
{
  ...
  "cases": [
    { "args": { "arr1": "[3, 5, 1, 6, 8, 7]", "arr2": "[3, 1, 8, 7]" },   "expected": "[-1, 5, -1, 8]" },
    { "args": { "arr1": "[5, 9, 7, 8, 1]",     "arr2": "[5, 9, 7]" },      "expected": "[-1, -1, 9]" },
    { "args": { "arr1": "[1, 2, 3]",            "arr2": "[1, 2, 3]" },      "expected": "[-1, -1, -1]" },
    { "args": { "arr1": "[3, 2, 1]",            "arr2": "[3, 2, 1]" },      "expected": "[-1, 3, 2]" },
    { "args": { "arr1": "[5]",                  "arr2": "[5]" },            "expected": "[-1]" },
    { "args": { "arr1": "[1, 3, 2]",            "arr2": "[3, 2]" },         "expected": "[-1, 3]" },
    { "args": { "arr1": "[4, 1, 2]",            "arr2": "[1, 2]" },         "expected": "[4, 4]" }
  ]
}
```
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(
            new Solution().precedingSuperiorElement(arr1, arr2)
        ));
    }
}
```
...
| Shared predecessor | `arr1 = [4, 1, 2]`, `arr2 = [1, 2]` | `[4, 4]` | `4` precedes and dominates both queries. |
