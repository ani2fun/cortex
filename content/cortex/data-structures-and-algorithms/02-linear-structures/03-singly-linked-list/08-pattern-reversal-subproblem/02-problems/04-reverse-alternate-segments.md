...
- A trailing fragment shorter than `k` is left untouched
...
class Solution:
    def reverse_alternate_segments(self, head, k):
        # Your code goes here — precompute length // k chunks, use a
        # should_reverse toggle, reverse on True, skip on False.
        # On skip: left_bound = end (not start). Return new head.
        pass
...
public class Main {
    ...
    static class Solution {
        ListNode reverseAlternateSegments(ListNode head, int k) {
            // Your code goes here — precompute length / k chunks, use a
            // shouldReverse toggle, reverse on true, skip on false.
            // On skip: leftBound = end (not start). Return new head.
            return null;
        }
    }
