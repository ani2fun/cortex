---
title: '33. API design — REST, gRPC, GraphQL'
...
The table is a lookup; the actual decision is a short walk down a tree, and every branch turns on the *caller*, never on the technology's coolness:
...
---
flowchart TD
  start(["Who calls this API,<br/>and what does their<br/>screen or service need?"])
  start --> q1{"Public / third-party<br/>over the internet?"}
  q1 -->|"yes"| rest["REST + JSON<br/>cacheable, every language<br/>speaks it, OpenAPI docs<br/>(Stripe, GitHub v3)"]
  q1 -->|"no"| q2{"Internal service-to-service,<br/>you own both ends?"}
  q2 -->|"yes, latency &amp;<br/>type-safety dominate"| grpc["gRPC + Protobuf<br/>binary over HTTP/2,<br/>codegen stubs, streaming<br/>(Google internal)"]
  q2 -->|"no"| q3{"Many different front-ends<br/>each needing a different<br/>slice of a rich graph?"}
  q3 -->|"yes, round-trips hurt"| gql["GraphQL<br/>client picks exact fields,<br/>one round-trip<br/>(GitHub v4)"]
  q3 -->|"no, simple CRUD"| rest
```

<p align="center"><strong>The choice is not a beauty contest between protocols — it's a walk down this tree, and every fork asks about the client.</strong></p>
...
| **Idempotency** | `POST` retried after a timeout → duplicate | **idempotency keys** (store first result, replay on retry — [Lesson 19](/cortex/system-design/distributed-patterns/idempotency-retries-backoff)) |
...
### Why versioning is really a *compatibility* problem

"Evolve additively" sounds like a style tip until you see *why* it's forced. The moment you do a **rolling upgrade** — deploy the new code to a few servers at a time, watch, then continue — old and new code are running **simultaneously**, and old data and new data sit side by side. (Client apps are worse: a user may never install your update.) For the system to keep working, the contract has to bend in **both** directions at once (Kleppmann, *DDIA* 2e Ch5):

- **Backward compatibility** — *newer* code can read data written by *older* code. This is usually easy: you wrote the new code, so you know the old format and can handle it (worst case, keep the old reader around).
- **Forward compatibility** — *older* code can read data written by *newer* code. This is the trickier half: the old code must **ignore fields it doesn't understand** — and crucially, **preserve them on write-back**. If old code decodes a record into an object that silently drops unknown fields, then reads-modifies-writes, the new field is *lost*. That data-loss trap is the whole reason forward compatibility takes deliberate effort.

For an **API** specifically, the two directions land on different halves of the call (this is the framing most people miss):

| Scenario | Needs… on the request | …and on the response |
|---|---|---|
| **Older client → newer server** | backward compat (server reads old request) | forward compat (client reads new response) |
| **Newer client → older server** | forward compat (server reads new request) | backward compat (client reads old response) |

For internal RPC there's a simplifying assumption that collapses this: you almost always **deploy servers before clients**, so you only need *backward* compatibility on requests and *forward* compatibility on responses. Across an **organizational boundary** — a public API — you get no such luck: you don't control the clients and can't make them upgrade, so compatibility must hold **indefinitely**, and when a real break is unavoidable you end up running multiple API versions side by side. That is exactly why Stripe date-pins each account and has kept decade-old integrations alive.
...
---
sequenceDiagram
  participant New as New code (knows discount)
  participant Old as Old code (no discount)
  Note over New,Old: Additive change — discount is a new OPTIONAL field with a default
  New->>Old: Forward compat: Old reads data written by New
  Note over Old: Old ignores the unknown discount field and preserves it on write-back
  Old->>New: Backward compat: New reads data written by Old
  Note over New: discount is absent, so New fills in the default (e.g. 0)
  Note over New,Old: Both directions hold, so old and new safely coexist mid-rollout
```

<p align="center"><strong>An additive change (a new optional field with a default) keeps both directions working, so old and new code coexist safely during a rolling upgrade. A rename, a removed required field, or a narrowed type breaks one of these arrows.</strong></p>

### What the wire format buys you: JSON vs Protobuf vs Avro vs Thrift

The *envelope* you encode in decides how cheaply those arrows hold. The trade-off is **self-describing-but-bulky** versus **compact-but-schema-bound** (*DDIA* 2e Ch5):

| Format | Schema | Size / speed | Evolution model | Best for |
|---|---|---|---|---|
| **JSON / XML** | none (or optional, e.g. **OpenAPI / JSON Schema**) | verbose text; field names repeated in every payload | add optional fields, ignore unknown ones — by *convention*, not enforced; no integer/float distinction, no native binary (Base64 hack), big-int precision loss in JS | **cross-organization interchange**, browsers, public APIs |
| **Protocol Buffers / Thrift** | mandatory `.proto`/IDL, codegen | compact binary; field **tag numbers** instead of names on the wire | add a field with a **new tag** → old code skips the unknown tag (forward) and new code fills a **default** for the missing field (backward); you may rename freely (names aren't on the wire) but **never reuse a tag number**; changing a 32→64-bit int risks **truncation** when old code reads new data | **internal service-to-service** (gRPC) |
| **Avro** | mandatory schema; **no tag numbers** | most compact (just concatenated values — nothing on the wire identifies a field) | reader uses **both** the writer's schema and its own, matching fields **by name**; you may add or remove **only fields that have a default**; `null` needs a `union` type | big data files, **Kafka + a schema registry**, dynamically generated schemas |

Two non-obvious consequences fall out of this table:

- **Protobuf's tag numbers are the contract**, not the field names. The encoded bytes never mention `userName`; they mention field `#1`. So you can rename a field freely, but you can *never* repurpose tag `#1` for a different field — old data still out there means tag `#1`, and reusing it silently corrupts reads. (Reserve retired tags in the schema so nobody reuses them by accident.)
- **Avro carries nothing on the wire to identify fields** — a string and an integer look identical in the bytes — so the reader *must* know which schema the writer used. In a Kafka pipeline you don't ship the schema with every message (it would dwarf the data); instead a **schema registry** stores every version, and the message carries a tiny schema ID. This is why "JSON or Protobuf or Avro?" in a message bus is usually answered by "whatever your schema registry checks compatibility for."

This is also the seam between this chapter and message-passing systems: a message broker (Kafka, SQS, Pub/Sub) doesn't care about your encoding — a message is just bytes with metadata — so the *same* backward/forward rules govern your events, and a republishing consumer must **preserve unknown fields** for exactly the read-modify-write reason above (drop a field you couldn't parse and you erase it for everyone downstream).
...
One trap underlies all three request/response styles: **a remote call is not a local function call, no matter how much the framework hides it.** A local call takes roughly constant time; a network call's latency is wildly variable and it can *fail in ways a function never does* — the request can arrive but the response get lost, which is precisely the duplicate-charge hazard idempotency keys exist to defuse. (Part of REST's appeal, in fact, is that it's honest about this: it treats a call as state transfer over a network, not as a method invocation pretending to be local.) When that mismatch bites hard — the caller doesn't need an answer *right now*, or the receiver may be down, or one event should fan out to many consumers — drop the synchronous request/response model entirely and pass a **message through a broker** (Kafka, SQS, Pub/Sub). The broker buffers when the consumer is overloaded, redelivers after a crash, decouples sender from receiver, and removes the need for service discovery on the send path. This is exactly why a payments API issues **webhooks** for asynchronous events (`charge.succeeded`) instead of making you poll: Stripe *can't* call your function, so it publishes an event you consume on your own time. The same backward/forward compatibility rules from §3 govern those events — a message is just bytes the broker doesn't interpret — which is why event-driven shops put a **schema registry** in front of the bus.

## 7. Edge cases and failure modes

- **Breaking changes break clients silently.** Removing a field, making an optional field required, or narrowing a type will break consumers you can't see. The *exact* rules depend on the encoding (§3): in Protobuf/Thrift you may rename freely but must **never reuse a tag number**, and a 32→64-bit widening can **truncate** when old code reads new data; in Avro you may add or remove **only fields that carry a default**. Across all of them, the silent killer is **dropping unknown fields on a read-modify-write** — old code that doesn't preserve a field it can't parse will erase it. Evolve **additively**; when a true break is unavoidable, **version** (date-pin like Stripe, or `/v2`) and run old and new in parallel with a deprecation timeline. Never mutate a published field in place.
...
- **Non-idempotent POST + retries = duplicates.** A lost response makes the client retry and double-create / double-charge. Require **idempotency keys** on unsafe mutating endpoints; tie the key to the intent, store-first-replay-later (Lesson 19).
