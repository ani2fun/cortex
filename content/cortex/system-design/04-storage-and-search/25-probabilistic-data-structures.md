---
title: '25. Probabilistic data structures'
...
**Bloom filter** (Burton H. Bloom, *Space/time trade-offs in hash coding with allowable errors*, CACM 1970). A bit array of **m** bits and **k** independent hash functions. *Insert*: hash the item k ways, set those k bits to 1. *Query*: hash k ways; if **all** k bits are 1, answer "probably present"; if **any** is 0, answer "definitely not present." It can never miss a member (you only ever set bits, never clear them), so a "no" is always correct — but unrelated items can collectively set all of some item's bits, yielding a **false positive**. The false-positive rate is `(1 − e^(−kn/m))^k`, minimized by choosing `k = (m/n)·ln 2`; at the optimum you need about `9.6` bits per element for a **1% FP rate** — and that's *independent of how big the items are*, which is the whole trick. *Designing Data-Intensive Applications* gives the same idea as a field rule of thumb: budget **about 10 bits per key for a 1% false-positive rate**, and the rate drops **roughly tenfold for every 5 extra bits** you spend — so ~15 bits/key buys ~0.1%, ~20 bits/key ~0.01%. (DDIA's worked Bloom filter even uses a concrete `handbag → (2, 9, 4)` insert and a `handheld → (6, 11, 2)` query that hits a 0-bit and so is *definitely absent* — exactly the §2 light-switch picture.) Standard Bloom filters don't support deletion. Of the three structures here, the Bloom filter is the one DDIA covers in depth (in the LSM/SSTable context below); HyperLogLog it mentions only in passing for stream cardinality, and Count-Min Sketch not at all — so the HLL and CMS details here lean on the original papers and established practice.
...
**Q1: "How many unique visitors today?"** — cardinality → **HyperLogLog.** Say the true answer is **50,000,000** distinct visitor IDs. The exact `Set` would cost ~400 MB *just for the raw 8-byte IDs* (realistically 1–2 GB with overhead). The HLL costs **12 KB** and reports, say, 50.2M — within its **0.81%** standard error, i.e. roughly **±405,000**. For a dashboard that says "≈50M visitors," that error is invisible and the memory saving is ~100,000×. This is precisely how it's used in practice: in **Redis** you `PFADD visitors:2026-06-21 <id>` on every request and `PFCOUNT` for the dashboard — and because two HLLs **merge by taking the max of each register**, `PFMERGE` rolls seven daily counters into a weekly-unique figure *without re-scanning a single ID*. Analytics engines expose the same trick: **Presto/Trino** `approx_distinct(user_id)` and **BigQuery** `APPROX_COUNT_DISTINCT` run an HLL under the hood, trading ~1–2% error for a query that finishes in seconds instead of shuffling billions of IDs to count them exactly.
...
This is the *same* shape as the single most common production Bloom filter you will ever touch — the one **DDIA** spends a figure on. An **LSM-tree** storage engine (Cassandra, RocksDB, Bigtable/HBase, ScyllaDB) keeps a key's data spread across many on-disk **SSTables**, so reading a key — *especially one that doesn't exist, or was last written long ago* — would otherwise mean checking file after file. Each SSTable carries a small Bloom filter over its keys. On a read the engine asks each filter first: a **"definitely not present"** lets it **skip that SSTable's disk seek entirely**, and only a **"maybe present"** costs it a real lookup through the sparse index and block — where a 1% false positive means just a little wasted work, never a wrong answer. As DDIA puts it, "in the context of LSM storage engines, false positives are no problem." The CDN's "is this URL new?" and Cassandra's "could this SSTable hold my key?" are *literally the same question*, answered the same way: a cheap negative that saves an expensive lookup. (Note the limit DDIA flags: Bloom filters help **point** lookups but **not range queries** — you can't hash a range.)

**Q3: "What are the top 10 trending search terms this minute?"** — frequency → **Count-Min Sketch** feeding a small top-K heap. Each search term increments its d cells; the heap keeps the current heaviest. The sketch is a few hundred KB regardless of how many *millions* of distinct queries fly by. This is the **heavy-hitters** pattern, and it's everywhere a firehose needs a leaderboard: trending hashtags, the top talkers hammering a rate limiter, the busiest source IPs in a DDoS, the **top-100 most-clicked ads in the last minute** that *System Design Interview Vol. 2* builds in its Ad Click Aggregation chapter (1 billion clicks/day across 2 million ads — far too many counters to keep exactly per minute).

**A second failure case — when the *frequency* answer must be exact.** That same Ad Click system carries a constraint the trending-terms dashboard does not: its counts feed **ad billing and real-time bidding**, so "correctness of the aggregation result is important." A Count-Min Sketch only ever **over**counts — perfect for "*which* ads are hot" (a slightly inflated rank is harmless) but disqualifying for "*how much* do we charge this advertiser" (you'd bill for clicks that never happened). The split is the §4 thesis in one system: use the sketch for the **ranking** query where one-sided error is harmless, and keep an **exact** count (idempotent, deduplicated, stored) for the **billing** query where it isn't. *Match the structure's error direction to what the question can tolerate* — and when the answer must be exact and auditable, no sketch will do.
...
And here is Q1's HyperLogLog, made physical — how one item's hash turns into one register update, the coin-flip intuition of §2 written out:
...
---
flowchart TB
  item(["item: user_42"]) --> hash["hash to 64 bits<br/>1101 00010110..."]
  hash --> split{"split the bit string"}
  split -->|"first p bits<br/>pick the bucket"| bucket["bucket = 1101 → register #13"]
  split -->|"remaining bits<br/>count leading zeros"| run["00010110... → 3 leading zeros<br/>run value = 3 + 1 = 4"]
  bucket --> reg["register[13] ← max(register[13], 4)<br/>keep only the longest run seen"]
  run --> reg
  reg --> est["combine all m registers by harmonic mean<br/>cardinality estimate, std err = 1.04/√m"]
```

<p align="center"><strong>HyperLogLog: the first bits choose a bucket, the rest contribute a leading-zero "streak," and each register only ever remembers its longest one. A run of 20 zeros is the fingerprint of ~a million distinct items, the same way 20 heads in a row betrays a million coin flips.</strong></p>
...
A framing worth keeping from **DDIA**: when stream processors reach for Bloom filters, HyperLogLog, and percentile estimators, the approximation is **"merely an optimization,"** not evidence that streaming is inherently lossy. The sketch is a *deliberate trade you control* — you pick the error and pay flat memory for it — not a defect you're stuck with. That is exactly why the choice is an engineering decision (clauses 1 and 2 above) rather than a compromise: reach for a sketch when the trade pays, and reach for the exact structure when it doesn't.
...
- **[Trino — `approx_distinct`](https://trino.io/docs/current/functions/aggregate.html#approx_distinct)** (and Presto, Spark's `approx_count_distinct`, BigQuery's `APPROX_COUNT_DISTINCT`) — HyperLogLog in your SQL prompt: `SELECT approx_distinct(user_id) FROM events` answers "how many uniques?" over billions of rows in seconds at ~2.3% error, instead of shuffling every distinct ID to count it exactly. The §4 Q1 trade, one keyword away.
- **DDIA, "Stream Processing" (Ch. 11)** — Kleppmann's framing of the whole family: stream analytics uses "Bloom filters for set membership, HyperLogLog for cardinality estimation, and various percentile estimation algorithms," and stresses that this approximation is **"merely an optimization"** — not proof that stream processing is lossy. The §6 mindset, stated plainly.

---

> **Next:** [26. Time-series databases](/cortex/system-design/storage-and-search/time-series-databases) — sketches shine on *firehoses*, and one firehose is so universal it earned its own database shape: metrics and events stamped with time, arriving append-only, queried by range, and mostly never updated. Next we'll see why time-series databases (Prometheus, InfluxDB, TimescaleDB) lay data out **by time**, downsample the old stuff to keep it cheap, and lean on the very LSM-tree and sketch ideas we just built.
