...
A **B-tree** (in practice a **B+tree**) is a balanced, n-ary search tree invented by Rudolf Bayer and Edward McCreight at Boeing in 1970. Each node is a fixed-size **page** (4 KiB traditionally; Postgres now 8 KB, InnoDB 16 KB) holding many sorted keys; leaves hold the data (or pointers to it) and are linked together for range scans. The number of child references per page is the **branching factor** — typically several hundred — which is why the tree stays so shallow: a four-level tree of 4 KiB pages with a branching factor of ~500 already addresses on the order of 250 TB. A lookup walks root → internal → leaf, typically **3–4 page reads** for huge tables (and the upper levels stay cached). A write finds the target leaf and **modifies the page in place**, writing the whole page back; if the page overflows it **splits** down the middle into two half-full pages and pushes the median key up into the parent (a split can cascade all the way to the root, which is how the tree grows a level). Crucially: updates are *random* I/O, and to change a few bytes you rewrite a whole page.

That split is the one structural event worth picturing, because it's where a B-tree pays to keep itself sorted:
...
---
flowchart TB
  subgraph before["BEFORE — insert 334 into a full leaf"]
    direction TB
    p1["parent: … | 333 | 345 | …"]
    f1["FULL leaf [333..345]<br/>333 335 337 340 345"]
    p1 -->|"keys 333–345"| f1
  end
  before -. "leaf is full → split on median 337" .-> after
  subgraph after["AFTER — split into two half-full leaves"]
    direction TB
    p2["parent: … | 333 | 337 | 345 | …"]
    lA["leaf [333..337]<br/>333 334 335 337"]
    lB["leaf [337..345]<br/>340 345"]
    p2 -->|"keys 333–337"| lA
    p2 -->|"keys 337–345"| lB
  end
```

<p align="center"><strong>A B-tree splits a full leaf in place: the median key (337) is promoted into the parent and the rest fans into two half-full pages. Reads stay a clean tree-walk, but every split is more random page rewrites — and those half-full pages are exactly the space amplification (fragmentation) a B-tree carries.</strong></p>
...
| **Bloom filter** | a tiny probabilistic index per SSTable that answers "this key is *definitely not* here" so reads can skip files (see [Lesson 25](/cortex/system-design/storage-and-search/probabilistic-data-structures)) |
...
Two details make this concrete. First, the memtable doesn't have to be a hash map — real engines use an *ordered* structure (a red-black tree, skip list, or trie) so it can absorb writes in any order yet be flushed to disk already sorted; RocksDB and LevelDB default to a skip list. Second, the bloom filter is a *point-lookup* accelerator only: it answers "is key `k` in this SSTable?" and lets a read **safely skip** any file it says no to. It does **nothing for range scans** — you can't hash every possible key in `[a, z)` — so an LSM range query must open and merge across *all* the candidate SSTables in parallel, which is precisely why range reads are the LSM's weak spot (and the tombstone trap in §4 is a range read gone bad).

How aggressively compaction runs is itself a tunable amplification knob, and the two classic strategies sit at opposite corners of the triangle:

- **Size-tiered compaction** merges several similar-sized SSTables into one bigger one (e.g. four ~256 MB files become one ~900 MB file — under 1 GB because overwrites and tombstones are dropped). It writes less often, so it's kinder to **write** amplification, but it leaves more overlapping files around (worse **read** and **space** amplification, especially during a merge). This is Cassandra's traditional default.
- **Leveled compaction** keeps fixed-size SSTables organized into levels where each level is ~10× the one above and files within a level have non-overlapping key ranges. A read therefore touches *far fewer* files (better **read** amplification) and stale data is reclaimed sooner (better **space** amplification), at the cost of more frequent re-copying (more **write** amplification). This is LevelDB's and RocksDB's signature strategy.

The rule of thumb: size-tiered favours write-mostly workloads; leveled favours read-heavy ones. Most engines let you choose per table — the same "pick the amplification you can afford" decision, one level down.
...
Write amplification is the cleanest place to *see* the two bets diverge — the same logical change takes a wildly different path on disk:
...
---
flowchart LR
  app(["change 80-byte row"])
  subgraph bt["B-tree — pay NOW (random)"]
    direction TB
    btw["rewrite whole 16 KB page<br/>(+ WAL + double-write)"]
    btn["≈ 200× bytes written,<br/>at a random disk location"]
    btw --> btn
  end
  subgraph lsm["LSM-tree — pay LATER (sequential)"]
    direction TB
    l_w["append to WAL + memtable<br/>(tiny, sequential)"]
    l_f["flush memtable → L0 SSTable"]
    l_c["compaction rewrites the byte<br/>once per level (L0→L1→…→Ln)"]
    l_w --> l_f --> l_c
  end
  app --> btw
  app --> l_w
...
<p align="center"><strong>Both engines write your data more than once (this is write amplification). The B-tree pays it <em>up front and randomly</em> — a full page per row touched. The LSM pays it <em>later and sequentially</em> — a cheap append now, then re-copies during background compaction. DDIA's definition: total bytes written to disk ÷ bytes a plain append-only log would have written.</strong></p>
...
- **Compaction can't keep up (LSM).** If the write rate outruns compaction, SSTable count climbs, read amplification rises, and you hit a latency cliff (and "too many open files"). When the memtable fills faster than it can be flushed, many engines (RocksDB included) apply **backpressure** — they deliberately stall or suspend writes (and sometimes reads) until disk catches up, so a write firehose can manifest as sudden latency spikes rather than data loss. Compaction is not free background magic — you must provision I/O and CPU headroom for it, or do the "gossip dance" by hand. (Note the asymmetry while benchmarking: an *empty* LSM has all disk bandwidth free for writes because no compaction is running yet; the real write amplification only shows up once the tree is large enough that new writes must share bandwidth with compaction — so short benchmarks flatter LSM.)
...
- **The B-tree random-write ceiling.** On any storage, random writes are slower than sequential. On an HDD that's mechanical (the head must seek); on flash it's subtler but real — SSDs read/write in ~4 KiB pages but can only *erase* in ~512 KiB blocks, so scattered random writes leave blocks half-stale and force the controller's **garbage collector** to relocate live pages before erasing. That GC both steals write bandwidth and wears the cells out faster, so a B-tree under a high-cardinality insert firehose hits this wall harder over a drive's lifetime — the original reason write-heavy systems reached for LSM (§4, §6).
- **Where the B-tree quietly wins (correctness, not just reads).** Because each key lives in exactly *one* place on a B-tree (the page that owns it), it's natural to attach locks to that page and offer strong transactional isolation — a big reason B-trees dominate the relational OLTP world. An LSM, where a key's versions are smeared across the memtable and several SSTables, has to work harder for the same guarantees. Conversely, the LSM's **immutable** SSTables make consistent **snapshots and backups** almost free (record which files existed at time *T* and don't delete them); snapshotting a B-tree whose pages are overwritten in place is much harder — which is exactly why copy-on-write B-trees like LMDB exist.
