...
**Combining terms is set arithmetic on posting lists.** A query like `quick AND fox` is the **intersection** of `quick`'s posting list and `fox`'s — the docs in *both*. Because posting lists are stored **sorted by document ID**, you intersect them with a merge-style walk (advance the smaller cursor), so the cost is proportional to the lists' lengths, not the corpus. DDIA notes a second representation: if document IDs are sequential, each posting list becomes a **sparse bitmap** — the *n*-th bit is 1 if document *n* contains the term — and an AND-query is then a literal **bitwise AND of the two bitmaps**, which run-length encoding makes blisteringly fast. `OR` is the union; `NOT` removes a list. The same machinery powers faceted filters ("red AND in-stock AND under \$50"): each facet is a posting list, and the filter is a chain of intersections.
...
The query `quick fox` first **finds** candidates — the intersection of the two posting lists — then **ranks** them. `quick`'s list is `{D1, D3}`, `fox`'s is `{D1, D3}`; their AND is `{D1, D3}` (D2 never enters the picture):
...
---
flowchart LR
  subgraph DOCS["documents → analyzer"]
    d1["D1: quick brown fox"]
    d2["D2: lazy brown dog sleeps"]
    d3["D3: quick quick quick<br/>fox fox runs"]
  end
  ...
  d2 --> tb
  d3 --> tq & tf
  tq --> AND{{"AND<br/>intersect<br/>sorted lists"}}
  tf --> AND
  AND --> hits["candidates: D1, D3<br/>(D2 skipped entirely)"]
  hits --> bm25["rank by BM25"]
  bm25 --> out(["D1 ▸ D3"])
```

<p align="center"><strong>Find then rank: an AND query intersects the sorted posting lists to get candidates, and only those candidates are scored. The corpus is never scanned.</strong></p>
...
## 6. Beyond exact terms — fuzzy search and autocomplete

An inverted index answers *"which docs contain this exact term?"* But real users mistype, and they want suggestions *as they type*. Two specialized structures handle these.

**Fuzzy search — tolerating typos with edit distance.** A search for `kitten` should still surface `kitchen` or a misspelled `kittan`. The yardstick is **edit distance** (Levenshtein distance): the minimum number of single-character insertions, deletions, or substitutions to turn one word into another. `kitten → kitchen` is distance 2; a single typo is distance 1. The naïve approach — compare the query against every term in the dictionary — is hopeless when the dictionary has millions of terms. DDIA describes how **Lucene** does it efficiently: it stores its set of terms as a **finite-state automaton over the characters in the keys — essentially a trie** — and compiles it into a **Levenshtein automaton**, a machine that accepts exactly the terms within a given edit distance of the query. Walking that automaton against the term dictionary finds all fuzzy matches without scanning every term one by one. This is why Elasticsearch's `fuzziness` parameter (`AUTO`, `1`, `2`) is cheap enough to enable in production. The cost: edit distance ignores meaning, so it catches typos but not synonyms — *"close my account"* will never fuzzy-match *"cancel subscription."* Closing that gap needs **semantic search**, which indexes the *meaning* of text as **vector embeddings** and ranks by nearness in that space — a different structure (an approximate-nearest-neighbour index) and a topic of its own.

**Autocomplete — the trie (prefix tree).** When a user types `tr`, you must instantly return the most popular completions: `true`, `try`, `tree`. An inverted index keys on *whole* terms, so it's the wrong tool for *prefix* lookup. The right structure is a **trie** (pronounced "try," from re**trie**val): a tree where each node is a character, the root is the empty string, and the path from the root to a node spells a prefix. All words sharing a prefix share a path, so finding the node for `tr` is just *p* hops (one per prefix character), and every completion hangs in the subtree below it. Attach a **frequency** to each terminal word, and autocomplete becomes: walk to the prefix node, gather the subtree's words, sort by frequency, return the **top-k** (k is small — 5 to 10 suggestions).
...
flowchart TB
  ...
  tr --> tre["tre"]
  tr --> tru["tru"]
  tr --> try["try ✓<br/>freq 29"]
  tre --> tree["tree ✓<br/>freq 10"]
  tru --> true["true ✓<br/>freq 35"]
  to --> toy["toy ✓<br/>freq 4"]
  w --> wi["wi"]
  wi --> win["win ✓<br/>freq 12"]
  wi --> wish["wish ✓<br/>freq 8"]
  tr -. "cached top-k:<br/>true(35), try(29)" .-> topk["return top 2"]
```

<p align="center"><strong>A trie for autocomplete. Typing <code>tr</code> walks to that node in 2 hops; its subtree holds <code>true</code>, <code>try</code>, <code>tree</code>, ranked by frequency. Caching the top-k at each node turns the lookup into O(1).</strong></p>

The naïve trie walk costs `O(p)` to find the prefix plus `O(c log c)` to sort the *c* completions in the subtree — and in the worst case (an empty prefix) that subtree is the whole trie. SDI's *Design a Search Autocomplete System* gives the two optimizations that make it production-grade: **(1) cap the prefix length** (users rarely type 50+ characters), turning prefix lookup into effectively `O(1)`; and **(2) cache the top-k completions directly on each node**, so a lookup is "walk to the node, read its cached list" — no subtree traversal, no sorting, `O(1)` total. The cost is space (every node stores k entries) and **staleness**: you don't rebuild the trie on every keystroke (billions of queries a day would melt it). Instead, an **offline data-gathering pipeline** aggregates query logs — hourly for Twitter-style trends, weekly for stabler corpora — and rebuilds the trie in the background, then ships it to an in-memory **trie cache**. Same lesson as the inverted index: pay to build ahead of time, serve reads instantly.
...
| **Prefix / autocomplete** | scan + filter (slow) | poor fit — use a **trie** with cached top-k |
| **Typo tolerance** | manual | **fuzzy** via edit-distance (Levenshtein automaton) |
| **Ranking** | none | TF-IDF / BM25 relevance |

The first trade is **scan vs. index**: a scan needs zero preparation and handles arbitrary patterns (substrings, regex), which is why `grep` and code search lean on it (or on a trigram index); an inverted index pays upfront and can *only* match the tokens it indexed, but then answers ranked term queries over millions of documents in milliseconds. The second trade is **the ranking function**: TF-IDF is simpler and fine for small or trusted corpora, but BM25's saturation and length-normalization are nearly free and far more robust — there's little reason to choose TF-IDF today, which is why every major engine defaulted away from it. The third is **freshness vs. throughput**: immutable segments make indexing fast and search near-real-time, but you pay an ongoing **merge** cost (the LSM tax from lesson 22). And the fourth, from §6, is **one index per query shape**: term lookup → inverted index, substring → trigram, prefix → trie, typo → Levenshtein automaton. No single structure does all four; production search stacks several and routes each query to the right one.
...
> <summary>Solution</summary>
>
> **(a) Inverted index** (Lucene / Elasticsearch / OpenSearch) with **BM25** ranking — the canonical full-text case: tokenized terms, ranked relevance, millions of docs. **(b) A trigram / ngram index**, not a standard inverted index — you're matching a **substring** inside identifiers (`TODO(alice)` may sit inside larger tokens), which whole-token indexing can't do. This is exactly GitHub's Blackbird problem from §1. **(c) Neither — it's not a search problem at all.** "How many distinct users" is a **cardinality** question → **HyperLogLog** from [lesson 25](/cortex/system-design/storage-and-search/probabilistic-data-structures), in 12 KB. The meta-lesson of this whole part: match the data structure to the *shape of the question*, and don't force one tool to do another's job.
>
> </details>

> **Exercise 4 — Autocomplete and a typo.**
> (a) A user types `tr` into a search box that must return the 2 most popular completions instantly. Why is an inverted index the wrong structure, what is the right one, and which optimization makes the lookup O(1)? (b) The user then misspells a query as `kittan`. What technique surfaces `kitten` anyway, and what is its one blind spot?
>
> <details>
> <summary>Solution</summary>
>
> **(a)** An inverted index keys on *whole* terms, so it can't answer a *prefix* query like "everything starting with `tr`." The right structure is a **trie (prefix tree)**: walk `t → tr` in 2 hops, and the completions hang in the subtree. The naïve version still traverses that subtree and sorts by frequency (`O(c log c)`); the optimization is to **cache the top-k completions on each node** (from SDI's autocomplete design), so the lookup becomes "go to the node, read its cached list" — `O(1)`. You pay for it in space and in **staleness**, since the trie is rebuilt offline from query logs, not on every keystroke. **(b)** **Fuzzy search via edit distance** — `kittan` is one substitution (distance 1) from `kitten`. Lucene finds such matches by compiling its trie-shaped term dictionary into a **Levenshtein automaton** rather than comparing against every term. The blind spot: edit distance is purely lexical, so it catches *typos* but never *synonyms* — `cancel` will not fuzzy-match `terminate`. That gap is what semantic / vector search exists to close.
>
> </details>
...
- **[Lucene's `FuzzyQuery` and finite-state automata](https://blog.mikemccandless.com/2011/03/lucenes-fuzzyquery-is-100-times-faster.html)** (Mike McCandless) — how Lucene's fuzzy search went ~100× faster by compiling the term dictionary into a **Levenshtein automaton** instead of comparing every term. The §6 fuzzy-search mechanism, from a Lucene committer; pair it with the FST work behind it.
