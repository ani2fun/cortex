---
title: '23. Circuit breakers and bulkheads'
...
# 23. Circuit breakers and bulkheads
...
The two forms aren't interchangeable. A **thread-pool bulkhead** runs the downstream call on a *separate* thread, which buys you a hard timeout (the calling thread can walk away from a hung pool thread) and true isolation — but every call pays the cost of a thread hand-off and context switch, and you now have N pools to size. A **semaphore bulkhead** is just an atomic counter on the calling thread: nearly free, no extra threads, but it *cannot interrupt a stuck call* — if the call hangs, the permit it holds stays held until the call's own timeout fires. Hystrix made this exact choice explicit: it defaulted to thread-pool isolation precisely so a slow dependency couldn't pin a caller thread, and offered semaphores only for ultra-high-volume calls where the thread-switch overhead became the bottleneck. The picture below is the canonical thread-pool bulkhead — three dependencies, three sealed compartments:
...
gw: Service (request handlers) {
  shape: rectangle
}

bh_a: Bulkhead A - 20 threads {
  shape: rectangle
  style.fill: "#dbeafe"
}
bh_b: Bulkhead B - 10 threads {
  shape: rectangle
  style.fill: "#ede9fe"
}
bh_c: Bulkhead C - 10 threads {
  shape: rectangle
  style.fill: "#fef9c3"
}
...
<p align="center"><strong>One sick dependency (Recommendations) can saturate only its own 10-thread compartment. Fraud-check and Inventory keep their threads; the service stays up, degraded by exactly one feature.</strong></p>
...
A subtlety hides in that 2-second number: **a circuit breaker is only as good as the timeout behind it.** The breaker can only count a hung call as a failure once the call *returns* — and a call returns on timeout. If fraud-check's client had no timeout (or a 30-second one), each thread would sit blocked for 30 s before the breaker ever saw a "failure" to count, and the pool would be long gone before the window filled. The aggressive timeout is what turns an indefinite hang into a fast, *countable* failure; the breaker is what stops you from generating that failure over and over. Timeout, breaker, and bulkhead are three legs of the same stool — Nygard treats the bare timeout as the most fundamental stability pattern of all, the thing every remote call must have before any of the others matter.
...
Two notes. I track *consecutive* failures here for brevity; production breakers track a **rate over a sliding window** (cleaner under mixed traffic). And I use `System.nanoTime()` — a monotonic clock — not wall-clock time, so an NTP adjustment can't make the cooldown comparison misbehave (the same discipline as the rate limiter in [Lesson 22](/cortex/system-design/distributed-patterns/rate-limiting)).
...
- **The breaker hides the real problem.** A tripped breaker serving fast fallbacks looks *great* on a request dashboard — low latency, lots of 200s — while a critical dependency is on fire. **Alert on breaker state transitions**, not just on request error rates. An open breaker on a core dependency is a page, not a shrug. This is exactly what the famous **Hystrix dashboard** existed to surface: one animated circle per dependency — colour-coded by health, sized by traffic — with the breaker's state (open/closed) called out, so an operator could see *at a glance* which compartment had flooded. The lesson outlives the library: whatever you use, the breaker's own state must be a first-class signal on the wall, not buried under green request counts.
...
- **Retries stacked on top of a breaker.** If a client retries (Lesson 19) *and* a breaker sits underneath, make the retry policy respect the open circuit — don't retry into an open breaker, or you rebuild the retry storm one layer up. Breaker, retry, and timeout are a *set*; tune them together.
