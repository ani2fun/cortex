---
title: '17. Message queues and event streams'
...
# 17. Message queues and event streams
...
| **Offset** (logs) | a consumer's position in a partition — the bookmark. DDIA notes this is *literally* a log sequence number: the broker behaves like a leader database and a consumer like a follower replaying the replication log, resuming from its last recorded position after a disconnect |
...
One subtlety the term "offset commit" hides: in a log, the broker does **not** track an ack for every individual message. Because a partition is consumed in order, "I've processed everything up to offset 4012" summarises the entire history in one number. That single periodic checkpoint — instead of per-message bookkeeping — is a big reason logs hit such high throughput. The price is granularity: if the consumer crashes after processing offsets 4013–4015 but before checkpointing, it resumes at 4012 and reprocesses all three. The offset is *cheap* but *coarse*; the redundancy is exactly the at-least-once duplication you'll plan for in §4.

The relationship to *grasp* before anything else is how partitions, consumer groups, and fan-out fit together. Within **one** group, the broker hands each partition to exactly one member — so a group can run at most as many parallel consumers as the topic has partitions (a 12-partition topic caps a group at 12 working consumers; a 13th sits idle). Across **different** groups, every group receives the full stream independently — that is fan-out, and it's free because reading a message never deletes it. This one diagram is the whole "queue *and* broadcast in one system" trick:
...
flowchart LR
  ...
  subgraph G1["Consumer group: billing<br/>(load-balanced: 1 partition → 1 member)"]
    direction TB
    B0["consumer B0"]
    B1["consumer B1"]
  end
  subgraph G2["Consumer group: analytics<br/>(independent — gets EVERY message)"]
    direction TB
    A0["consumer A0"]
  end
  P0 --> B0
...
<p align="center"><strong>Inside a group, partitions are divided (work-sharing); each group is a separate "subscriber" that sees the whole topic (fan-out). DDIA: combine load balancing and fan-out by combining consumer groups.</strong></p>
...
That table is the **consumer** side of the story. There is a matching dial on the **producer** side, and beginners often forget it: a message you never durably stored can't be delivered any number of times. In Kafka the producer chooses how many replicas must confirm a write before it counts as sent (the `acks` setting), and the broker keeps a set of **in-sync replicas (ISR)** — followers that are caught up with the partition leader.

| Producer `acks` | Producer waits for… | Trade-off |
|---|---|---|
| `acks=0` | nothing (fire-and-forget) | fastest; a broker crash before the write lands = **silent loss** (the producer-side at-most-once) |
| `acks=1` | the leader only | a write the leader took but hadn't replicated is lost if the leader dies before a follower copies it |
| `acks=all` | every in-sync replica | slowest (bounded by the *slowest* ISR member) but durable — the message survives losing the leader |

So end-to-end delivery is two semantics multiplied together: the producer's durability (`acks`) *and* the consumer's ack/commit timing. "At-least-once" is only honest when **both** dials are set for it — `acks=all` on the way in, commit-after-processing on the way out. (Detail per SDI Vol 2; DDIA makes the same point more generally — durability is "moved to the broker," and brokers achieve fault tolerance by replicating messages.)
...
The honest cost of pull, per SDI Vol 2: when the log is empty, a naïve consumer spins — "anything yet? anything yet?" — burning CPU and network on empty replies. The standard fix is **long polling**: the consumer's fetch request *parks* on the broker for up to, say, 500 ms and returns the instant a message arrives (or empty at the timeout). You keep pull's back-pressure and batching while getting close to push's latency. Push, by contrast, is naturally low-latency but has no built-in answer for "consumers with diverse processing power" — a fast producer can bury a slow consumer unless the broker adds explicit flow-control.
...
Here is the whole loop as a sequence — including the crash that makes the ordering of those last two lines matter. Watch *when* the offset moves:
...
sequenceDiagram
    ...
    participant DB as Offset store
    P->>B: append charge #7782 (offset 41)
    B-->>P: acks=all → durable
    C->>B: fetch from offset 41
    B-->>C: charge #7782
    C->>C: charge_payment_api()  (do the work)
    Note over C: crash BEFORE commit
    C->>B: fetch from offset 41 (resume at last commit)
    B-->>C: charge #7782 again
    C->>C: already_processed(key)? → no-op
    C->>DB: commit offset 42
    Note over C,DB: committing LAST = at-least-once<br/>idempotency makes the redelivery harmless
```

The redelivery in steps 7–8 is not a bug — it is the *defining behaviour* of at-least-once, and the reason §4's idempotency check exists. If the consumer had committed offset 42 *before* the crash, the charge would have been lost (at-most-once). The diagram is really a picture of one decision: where the `commit` arrow sits relative to the work.
...
**How much can a log actually hold?** DDIA frames a log as a *bounded circular buffer on disk*: old segments are deleted (or tiered to archive) once they age out, so a slow consumer that falls behind the oldest retained segment simply misses those messages. The capacity is larger than people expect. DDIA's back-of-the-envelope: a 20 TB drive with ~250 MB/s sequential write throughput takes roughly **22 hours** to fill at the *maximum* write rate — so even flat-out, the log buffers nearly a day; at realistic utilisation it holds days-to-weeks. Multiply by many disks across many brokers and the "buffer" is enormous. This is why the standard operational drill is *monitor consumer lag and let a human fix the slow consumer before it ages past the retention window* — there is real time to react. Newer brokers (Kafka tiered storage, Redpanda, WarpStream) push old segments to object storage to stretch retention further still.
...
- **Consumer lag.** If producers append faster than a consumer reads, its offset falls further behind real-time. A little lag is normal; *growing* lag is an alarm — your consumer is under-provisioned or stuck. Monitor lag (current offset vs log end offset), not just throughput. DDIA's reassurance: because the on-disk buffer is large (days, per §6), a lagging consumer usually gives a human ample time to react before it ages past retention — and crucially, **one slow consumer doesn't disrupt the others**, so you can even point an experimental consumer at a production log for debugging.
...
- **Poison messages.** One malformed message that throws on every parse will, under at-least-once, be redelivered *forever*. DDIA's worked version: a producer omits a required field, the consumer crashes parsing it, never acks, the broker resends it, the *next* consumer crashes — an infinite loop that wastes resources. Worse, on a **strongly ordered** log it is **head-of-line blocking** — nothing behind that offset can advance until the bad message clears. The fix is a **dead-letter queue (DLQ)**: after N failures, move the message aside (and alert on the DLQ — anything landing there is a bug to investigate). Always cap the retry count.
- **Load balancing + redelivery silently reorders a queue.** A subtle one from DDIA, specific to *queue*-style brokers with competing consumers: even if the broker tries to preserve order, redelivery breaks it. Picture consumer 2 crashing mid-`m3` while consumer 1 is already on `m4`; `m3` gets redelivered to consumer 1 *after* `m4`, so it processes `m4, m3, m5` — out of producer order. If messages are independent this is harmless; if they have causal dependencies it's a bug. The only clean fix in that model is one queue per consumer (i.e. don't load-balance). Logs sidestep this by giving each partition to exactly one consumer.
...
- **Unbounded retention.** A log keeps data for its retention window — set it too long (or to "forever") on a high-volume topic and you fill the disk. Tune retention by time *and* size; use **log compaction** for "latest value per key" topics (it keeps the most recent value for each key and garbage-collects older ones — ideal when the log is a changelog of current state rather than a history of events).
- **"Exactly-once" has a scope.** Kafka's exactly-once semantics (since **0.11**, 2017) are real for *Kafka-to-Kafka* read-process-write inside one transaction. The moment your side effect leaves Kafka — charging a card, sending an email, writing to a different database — you are back to needing idempotency on *that* boundary. DDIA prefers the honest name **effectively-once**: the framework can discard the partial output of a failed task *inside* its boundary, but it cannot un-send an email or un-charge a card. Exactly-once is a property of a closed loop, not a magic wire.
