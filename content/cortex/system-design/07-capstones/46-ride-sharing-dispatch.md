---
title: '46. Ride-sharing dispatch (capstone)'
...
# 46. Ride-sharing dispatch (capstone)
...
That's the whole capstone in one design choice. A ride-sharing dispatch system *looks* like it should be a database query — "find drivers within 2 km" — but the data is **alive**: millions of drivers, each emitting a new GPS position every few seconds (Uber's DISCO dispatch system was reported to process on the order of **hundreds of thousands of location writes per second**), and a rider who needs an answer in seconds. You cannot run a geo range-scan against a relational table at that write rate and read latency. So the heart of the system is a **geospatial index** — a cell grid held in memory, updated by the location firehose, queried with a **k-ring** to find nearby drivers in O(1).

This isn't an Uber-only shape. **Lyft** indexes drivers with **geohash** (a string-prefix cousin of H3, covered in §5); **DoorDash** and **Instacart** run the *same* nearest-available-courier matching over a fleet of Dashers; **Grab**, **Bolt**, and **Didi** all solve this geometry. Whenever you need "who's near this point, right now, out of a moving fleet" — dispatch, food delivery, scooter rentals, even Tinder's "people nearby" (which uses Google's **S2**) — the engine underneath is a geospatial cell index fed by a location firehose.
...
**Out of scope:** the routing/ETA engine's internals (we treat it as a service), payments ([Capstone 49](/cortex/system-design/capstones/payment-system)), fraud, and pooled/shared rides (a harder matching problem).
...
Two things jump out. First, the system is **write-dominated** by location updates (~25× the request rate), so the index must absorb a relentless stream of position changes — which is why you **batch/aggregate** updates and keep the index **in memory** (it's tiny — ~500 MB — because a driver's position is just a few bytes; the data is small but *furiously hot*). Second, the geo data is small enough that the challenge isn't storage, it's **update throughput and query latency** — exactly what a cell-grid index optimizes.

A reality check on these numbers: published designs land in the same order of magnitude. A "nearby friends" system at 10M concurrent users updating every 30 s yields ~334K location writes/s; Google Maps, with clients **batching** GPS into one push every ~15 s, cites ~200K updates/s average and ~1M at peak. The lesson is the same everywhere: the update rate is the **firehose that sets the architecture**, and the single biggest lever on it is the **report interval** — halving how often each driver pings halves the write load, at the cost of slightly staler positions. Most of the engineering lives on the *write* path, not the read path.
...
| Approach | Idea | Trade-off | Used by |
|---|---|---|---|
| **Range scan** (`lat/lon BETWEEN`) | query a bounding box in a table | dies at this write rate; no efficient "nearby" | — |
| **Geohash** | interleave lat/lon bits into a base-32 **string**; nearby points share a **prefix**; trim a char to widen | simple; a DB index of strings; but rectangular cells, and **boundary blindness** (close points can share no prefix) | **Lyft**, Bing, Redis, MongoDB |
| **Quadtree** | recursively split a region into 4 quadrants until each holds ≤ N points | **adapts to density** (fine downtown, coarse in deserts); in-memory; rebuild/update under churn is fiddly | Yext; Elasticsearch (with geohash) |
| **S2** (Google) | spherical cells linearised on a **Hilbert curve**; `Region Cover` tiles any shape | great hierarchy + **geofencing**; cell shapes vary by latitude | **Google Maps, Tinder** |
| **H3** (Uber) ✅ | **hexagonal** hierarchical cells | **uniform neighbour distance** (no diagonal bias); kRing ≈ growing circle; the ride-dispatch default | **Uber** |

A few details that decide between them:

- **Geohash** maps `(lat, lon)` to a string by interleaving their bits — `9q8yyk` is downtown San Francisco — so "nearby" becomes "shares a prefix," and a longer prefix means a smaller, closer cell. Precision is the string *length*: length 5 is a ~4.9 km × 4.9 km cell, length 6 ~1.2 km × 0.6 km, length 7 ~150 m × 150 m. To widen the search you literally **drop the last character** (length 7 → 6) and re-query a coarser cell. The catch is **boundary blindness**: the prefix guarantee is one-directional — a long shared prefix *does* mean two points are close, but two close points can share *no* prefix at all if they straddle a top-level split. The textbook example: La Roche-Chalais (`u888`) and Pomerol (`ezzz`) in France are **30 km apart yet share zero prefix**, because they fall on opposite sides of a major grid line. You fix it the same way H3 does — query the cell **plus its eight neighbours** (computable in O(1)) — so a driver just over the line is never missed.
- **Quadtree** is the one that **adapts to density**: it keeps splitting a quadrant only while it's crowded, so a dense downtown gets many tiny cells while an empty desert stays one big cell — the search depth self-tunes to where the drivers actually are. The cost is that it's an *in-memory tree* you build at startup and must carefully mutate (or periodically rebuild) as the fleet churns.
- **S2** linearises the globe onto a **Hilbert space-filling curve**, whose magic property is that points close on the 1-D curve are close on the sphere — so a 2-D nearest-neighbour search collapses to a 1-D range scan. Its `Region Cover` can tile an *arbitrary* shape (an airport polygon, a city boundary) with mixed cell sizes, which is why it's the tool of choice for **geofencing**.

The pattern, whichever index: **map each driver to a cell on update; to find nearby drivers, compute the rider's cell and query it plus its ring of neighbours (a k-ring)** — `O(1)` cell lookups instead of an `O(drivers)` scan. H3's hexagons win for dispatch because expanding the ring (`k=1`, then `k=2`, …) grows the search radius *evenly in all directions*, so "no driver in the immediate ring? widen by one" is clean — and the hierarchy lets you pick cell resolution by density (fine cells downtown, coarse in the suburbs), echoing the quadtree's density-adaptation without the per-region tree to maintain. The index is **sharded by region**, which is free here because a ride never spans cities.
...
**Why the ETA step is the expensive one.** Road-network ETA is a shortest-path search (a variant of **Dijkstra** or **A\***) over a graph of intersections and road segments — and the planet's road graph is far too large to hold in memory or search end-to-end. The standard fix (Google Maps, OSRM, Valhalla) is **routing tiles**: the world is cut into tiles, each a binary blob of the road graph for its patch, holding references to neighbouring tiles so the search can **stitch a larger graph together by loading tiles on demand** — only the handful of tiles the path actually touches ever enter memory. To make *long* routes tractable there's a **hierarchy of tiles**: fine tiles of **local roads**, medium tiles of **arterials**, and coarse tiles of **highways**, so a cross-town route uses detailed tiles near the two endpoints and jumps onto coarse highway tiles for the long middle stretch — the routing equivalent of H3's coarse-vs-fine resolution. For dispatch the saving grace is that we only need a *short* pickup ETA over a handful of nearby candidates, so the search stays in a few local tiles; live **traffic** (fed from the very location pings the system already collects) adjusts the edge weights so the ranking reflects current conditions, not the speed limit.
...
- **Hot cells in dense areas.** A stadium letting out, an airport, downtown at rush hour — one cell can hold a huge fraction of a city's drivers and absorb a request storm. This is the classic **hot spot** (a "hot shard," or for a single overloaded key a "hot key") that DDIA flags as the weak point of any **key-range sharding** scheme — and geographic sharding *is* key-range sharding over space. Mitigations: use H3's **finer resolution** in dense areas (smaller hexagons split one busy cell into many), and shard a hot cell across nodes. Note DDIA's caveat on the "add a random prefix to spread the key" trick: it splits the **write** load but not the **read** load — a reader now has to gather from every sub-key — so it's no free lunch for a cell that's hot on *queries*.
- **The location write firehose.** ~100M updates/s would crush per-update writes; **batch and aggregate** aggressively, keep the index in memory, and accept slightly stale positions (a driver 2 seconds out of date is fine — this is plain **eventual consistency**, where replication lag of a second or two is invisible to the rider). Most of the engineering is in making the *write* path cheap. This is also *why* a B-tree-indexed relational table is the wrong tool: B-trees turn scattered key updates into **random writes** (plus write-ahead-log **write amplification**), and disks do far worse on random writes than on the **sequential writes** a log-structured (LSM) engine batches them into — so a write-heavy firehose wants an in-memory map or LSM store, not a B-tree on disk.
- **Single-assignment under contention.** When demand spikes, many riders chase the same few nearby drivers; the atomic claim ([idempotency / compare-and-set](/cortex/system-design/distributed-patterns/idempotency-retries-backoff)) must be fast and correct, or you double-dispatch. A **compare-and-set** is only atomic if it runs against **linearizable** storage (a single-leader row, or a `WATCH`/`MULTI` in Redis, or a conditional write in DynamoDB) — leaderless eventual-consistency stores can't give you CAS without a consensus round. This is the one consistency-critical core amid an otherwise eventually-consistent system.
- **ETA at scale.** Ranking by real road ETA means the routing engine is queried per match against a live road graph + traffic — a heavy service that's often pre-computed/cached per region (and per routing tile) and approximated for the candidate set, then refined for the winner.
- **The surge feedback loop.** Surge is computed **per cell** over rolling windows from the same supply/demand signals the geo index already holds (available drivers vs. open requests). It's a **closed control loop**, and like any control loop it can **oscillate**: a price spike suppresses demand, demand collapses, the price drops, demand floods back, and it spikes again. You damp it (smooth over rolling windows, cap the per-step change) and harden it against **gaming** — e.g. drivers coordinating a mass log-off to manufacture scarcity.
...
---
flowchart LR
  geo["Geo index<br/>(per cell)"] -->|supply: available drivers| ratio{"demand ÷ supply<br/>per cell, rolling window"}
  req["Open ride requests"] -->|demand| ratio
  ratio -->|imbalance| price["Surge multiplier<br/>(smoothed, capped)"]
  price -->|higher price| riders["Riders<br/>(some wait / drop)"]
  price -->|higher earnings| drivers["Drivers<br/>(more come online,<br/>relocate to cell)"]
  riders -.->|demand falls| req
  drivers -.->|supply rises| geo
```

The dashed arrows are the feedback: surge nudges *both* sides of the ratio back toward balance — fewer riders, more drivers — which is exactly why an undamped loop overshoots and rings.
