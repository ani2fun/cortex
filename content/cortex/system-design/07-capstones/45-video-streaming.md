---
title: '45. Video streaming (capstone)'
...
**Out of scope:** live/low-latency streaming (real-time transcode — we focus on **VOD**, video-on-demand), DRM/content protection, recommendations ([Capstone 51](/cortex/system-design/capstones/recommendation-serving)), and the social layer.
...
- **Metadata** (`video_id → {owner, title, status, available renditions}`) in a relational/NoSQL DB — small, queried for listings and the player bootstrap. This is the **system of record**: the canonical truth, written first.
...
There's a useful distinction hiding in that split. In DDIA's vocabulary, the **raw upload and the metadata are the system of record** (the source of truth, written once by the creator), while **every transcoded segment is *derived data*** — a transformation of the source that exists purely to be served fast. The defining property of derived data is that *if you lose it, you can re-create it from the source* (the same way a cache or a search index can be rebuilt). That single fact is the licence for the whole design: because segments are regenerable, you can store them on cheap classes, cache them aggressively at the edge, drop unwatched renditions, even re-encode the whole catalogue when a better codec ships — none of it risks the original. The raw file is precious and kept safe; the renditions are disposable and kept *close*.

The **central design decision** is the **transcode-and-package pipeline**, and the three choices inside it:

1. **The bitrate ladder.** Encode the source into several renditions — e.g. 240p/400kbps, 360p/800kbps, 540p/1.5Mbps, 720p/2.5Mbps, 1080p/5Mbps (and 4K/15Mbps). More rungs = smoother adaptation but more storage and transcode cost. (Each rung is a full re-encode at a target resolution *and* bitrate — see the `LADDER` in §10.)
2. **Segmenting.** Chop each rendition into **2–10 second segments**, aligned to **GOP boundaries**. A GOP (Group of Pictures) is a self-contained run of frames that starts with a *keyframe* (a full image) followed by frames encoded as deltas against it; cut a segment on a GOP boundary and it's an **independently decodable unit** — the player can start, seek, or switch bitrate at any segment without needing the previous one. This is exactly what makes each segment both a cleanly switchable ABR unit *and* an immutable, independently-cacheable static file. (It's also why uploads chunk on GOP alignment: a resumable upload can retry one chunk in isolation.)
...
The two diagrams above show *topology* and *containers*. But the pipeline itself is best understood as a **DAG of processing stages** — a directed acyclic graph where each stage's output feeds the next, with independent branches fanning out for parallelism. This is the same **dataflow** structure DDIA describes for modern batch engines (Spark, Flink): assemble operators into a graph, run independent branches concurrently. It's also exactly how production systems model it — Facebook's video engine (SVE) and YouTube both use a DAG so creators can plug in optional stages (a watermark here, a custom thumbnail there). Here's the transcode DAG for one upload:
...
---
flowchart LR
  RAW[Raw upload] --> INSP[Inspect / validate]
  INSP --> SPLIT[Demux + split on GOP boundaries]
  SPLIT --> VID[Video track]
  SPLIT --> AUD[Audio track]
  SPLIT --> THUMB[Thumbnail extract]
  VID --> E240[Encode 240p]
  VID --> E720[Encode 720p]
  VID --> E1080[Encode 1080p]
  VID --> E4K[Encode 4K]
  AUD --> AENC[Encode audio]
  E240 --> SEG[Segment + package]
  E720 --> SEG
  E1080 --> SEG
  E4K --> SEG
  AENC --> SEG
  SEG --> MAN[Write HLS/DASH manifest]
  THUMB --> MAN
  MAN --> OUT[(Object storage to CDN)]
```

The fan-out in the middle — one video track splitting into four parallel encode stages — is where the **embarrassingly parallel** nature of transcoding lives (DDIA's exact term for work that shards cleanly with no coordination between shards). Each encode is an independent job; the farm can run all four rungs at once, and within a single rung it can split the source by GOP and encode the chunks in parallel too, so a 4K render that would take an hour single-threaded finishes in minutes.

Notice that the **viewer never touches your application** — they talk only to the **CDN**, fetching static manifests and segments. Your origin (object storage) is just the CDN's backstop for cache misses. That's the architectural payoff of "everything is a static, immutable segment": serving video becomes serving files, and serving files is the one thing CDNs do best. The transcode workers drain a **queue** ([message queues](/cortex/system-design/distributed-patterns/message-queues-and-streams)) and [autoscale](/cortex/system-design/production-operations/capacity-planning-and-autoscaling) with the upload firehose, completely independent of the serve path. A **log-based broker** (Kafka/Kinesis, in DDIA's taxonomy) is a natural fit here: jobs persist durably, survive a worker crash, and can be *replayed* — if a transcode worker dies mid-job, the message is still on the log and another worker picks it up.
...
- **Popularity is wildly skewed (cache the head).** A tiny fraction of videos get the overwhelming majority of views (a new release, a viral clip), so a modest edge cache covers most requests — the same Zipfian win as the [URL shortener](/cortex/system-design/capstones/url-shortener). The long, cold tail (most videos, watched rarely) doesn't belong at every edge — serve it from regional origins and keep edge capacity for the head.
- **Transcode is huge but embarrassingly parallel.** ~4.3M rendition-hours/day is a giant compute bill; you cut wall-clock time by **chopping each source into chunks (on GOP boundaries) and transcoding them in parallel** across an autoscaling farm (GPUs/ASICs help). Because each chunk is independent and the inputs are immutable, a failed chunk just gets **retried** — the job is idempotent (re-encoding the same chunk yields the same bytes), which is what lets the farm shrug off worker crashes without corrupting output. The bigger lever is *smarter* encoding: **per-title (and per-scene) encoding** — Netflix's "dynamic optimizer" picks a *custom* ladder for each video (and even each shot), spending bits where the picture is complex (a fast action scene) and saving them where it isn't (a static talking head), cutting bandwidth ~20% at the same perceived quality. And a one-time **catalogue re-encode** to a newer codec (H.264 → AV1) can permanently cut every future byte served — affordable precisely *because* segments are derived data you can always rebuild from the source.
...
- **The "thundering herd" premiere.** A simultaneous global release (a new season drop) spikes a single title to millions of concurrent viewers at once. **Pre-warm (pre-position) the edge** before the clock strikes: push the segments to every OCA/edge node *ahead* of demand so the premiere meets a hot cache instead of hammering the origin on a cold one. For a *predictable* spike (a scheduled release) this is straightforward; for an *unpredictable* one (a clip going viral), the edge fills reactively on the first few misses and is hot within seconds — the skewed-popularity win again.
- **Live changes the pipeline from batch to stream.** Everything above is **VOD (video-on-demand)**: a *bounded* input (the finished file) transcoded *offline* at leisure — a **batch** job in DDIA's terms. **Live** (a Twitch stream, a World Cup final, a keynote) flips it to an *unbounded* input transcoded in *real time* — a **stream** job. The pipeline shape survives (ingest → transcode → ladder → segment → manifest → CDN), but now it must run continuously with tight latency: short segments (1–2s, or LL-HLS/chunked-CMAF for sub-second), a transcode that keeps pace with the incoming feed, and no luxury of retrying a slow chunk — a late segment is a glitch, not a delayed file. Live also has *less* parallelism to exploit (you can't fan out a video you haven't received yet), and a sports final layers the thundering herd *on top*: millions of concurrent viewers on a stream that's being produced live, second by second.
...
- **Transcode failures and poison inputs.** A malformed or exotic upload can crash or hang a transcode worker; isolate jobs, time them out, retry with backoff ([Lesson 19](/cortex/system-design/distributed-patterns/idempotency-retries-backoff)), and quarantine repeat offenders so one bad file can't stall the farm.
