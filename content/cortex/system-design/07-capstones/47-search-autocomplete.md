---
title: '47. Search autocomplete (capstone)'
...
# 47. Search autocomplete (capstone)
...
In **2004**, Google launched an experimental opt-in feature called **Google Suggest** — as you typed in the search box, it offered completions. By **2010** it had become automatic and ubiquitous (briefly branded **Google Instant**, then simply **Google Autocomplete**), refreshing its predictions **as each character was typed**, no Enter key required. The same pattern is everywhere you type a query: **Amazon**'s search box completing "noise can…" → "noise cancelling headphones," **YouTube** finishing "how to…," your IDE and your phone keyboard. The names vary — *typeahead*, *search-as-you-type*, *incremental search*, *"design top-k"* in interview shorthand — but the shape is identical: predict the most likely completions of a growing prefix, ranked by what people actually search. The feature is so woven into how we search that we barely notice it — and that's exactly the bar. Autocomplete measurably *works*: studies find it cuts average search-task time from ~8.3 to ~4.7 seconds and **reduces keystrokes per query by ~42%**. But it only delivers that if it's *invisibly fast*. The Nielsen Norman Group's threshold for an interface feeling "instantaneous" is roughly **half a second** — and for type-ahead specifically you need far better than that, because the suggestions must keep up *with your typing*: a moderately fast typist hits a key every ~100–150 ms. Facebook's published bar for its own typeahead was **100 ms** end-to-end, *"otherwise it will cause stuttering"* — and that's the budget for the *whole* round trip, so to feel like magic rather than lag, the server's share has to land in **tens of milliseconds**.
...
Numbers ([estimation](/cortex/system-design/foundations/back-of-envelope-estimation)) — and the read rate is enormous because autocomplete fires *per keystroke*. The naïve count is one request **per character**: typing the 6-letter "dinner" fires `d`, `di`, `din`, `dinn`, `dinne`, `dinner` — and a ~20-character query would fire ~20 requests. Client-side **debounce** (fire only after a typing pause) collapses that to a handful, so assume **5 billion searches/day** at **~6 suggest calls** per query, and a popular-query "head" of **~10 million** queries worth trie-ing.
...
One endpoint, and the discipline is on the **client**: it **debounces** (waits ~50–100 ms after the last keystroke before firing, so typing "facebook" doesn't fire eight requests), it issues the call as a background **AJAX/fetch** so the page never reloads, and it must handle **out-of-order responses** — if the request for "fa" returns *after* the request for "fac", the client must show the latest, not whichever arrived last (a sequence number or "ignore stale prefix" check). The response is tiny and **cacheable at three layers**, each catching traffic before the one behind it:

1. **Browser cache** — suggestions barely change minute-to-minute, so the response carries a TTL and the browser reuses it. Google's real header is `Cache-Control: private, max-age=3600` — cached for an hour, and `private` means *this user's browser only, never a shared cache* (the hook for personalization later).
2. **Edge/CDN cache** — for *non-personalized* results the suggestions for "fa" are the same for most users, so a shared edge cache serves hot prefixes without touching the trie at all (the [edge-cache](/cortex/system-design/capstones/url-shortener) trick again). Different countries can be served different cached tries from a CDN near them.
3. **Trie cache** — the in-memory suggest service itself (Section 6).
...
The naïve trie query is three steps — find the prefix node `O(p)`, traverse its subtree to collect all valid completions `O(c)`, sort them and keep the top k `O(c log c)` — and the middle two are the problem: walking and sorting the whole subtree at request time is too slow for a popular short prefix (the subtree under "a" is enormous). Two optimizations crush it. First, **cap the prefix length** (users rarely type past ~50 chars), making the find step a small constant. Second — the **central design decision** — **precompute the top-k completions at every node**, a structure often called a **suggest tree**: each node stores a **weight-ordered list of the k highest-weighted queries in its subtree**. For example, the node for prefix `be` might cache `[best:35, bet:29, bee:20, be:15, beer:10]`. Now a suggest request is simply:

1. **Traverse** to the prefix node — `O(prefix length)`, a handful of pointer hops.
2. **Return its cached top-k list** — `O(1)`.

No subtree scan, no ranking at request time — both expensive steps collapse to constant time. That precomputation is what buys the tens-of-ms latency. It's paid for **offline**, by the data-gathering pipeline (Section 6): a **batch job aggregates query frequencies** (a MapReduce: emit `query → 1`, reduce to total counts) over recent **append-only analytics logs**, takes the **top queries**, and **builds the trie**, computing each node's top-k as it goes — bottom-up, a node's top-k is the merge of its children's top-k lists plus any query ending there, so it's one pass, not a re-scan per node. The finished trie is **serialized and published** to the in-memory suggest service, and the whole thing is **rebuilt periodically** (SDI assumes *weekly*; hourly/daily for fresher domains) — acceptable precisely because autocomplete is **staleness-tolerant**. The trade is explicit: *spend storage and rebuild cost to make every read trivially fast.*
...
Walking the **build path** component by component, because this end-to-end pipeline — not the trie alone — is what separates this capstone from general full-text search:

- **Analytics logs** — every search is appended to a raw, unindexed log (`query, timestamp`). Append-only is the cheapest possible write; nothing here is in query-able shape yet. At scale you don't even log *every* search: **sampling** keeps 1 of every *N* requests, since the *relative* popularity of the head is statistically intact from a sample, and logging 100% of billions of queries wastes storage and processing for no ranking benefit.
- **Aggregators** — a batch job (MapReduce / Spark) that rolls the firehose of raw rows up into `(query, frequency)` over a window. The cadence is the freshness knob: aggregate weekly for stable domains, every few minutes for fast-moving ones like Twitter/news.
- **Workers** — servers that take the aggregated `(query, weight)` table and **build the trie**, computing each node's top-k, on a schedule.
- **Trie DB** — durable storage for the built trie. Two shapes: serialize the whole tree as a periodic **snapshot** into a document store (e.g. MongoDB), or flatten it into a **key-value store** where *each prefix is a key* and its cached top-k list is the value — a hash-table view of the trie that shards and looks up trivially.
- **Trie Cache** — the distributed in-memory copy the suggest service actually reads, loaded from Trie DB and refreshed each build cycle. This is layer 3 of the caching stack from Section 4.
...
- **Trie sharding is skewed.** When the trie outgrows one server, the obvious shard is by **first character** ("a–m" on server 1, "n–z" on server 2; split further for more servers — second- and third-level prefixes get you past 26 shards). But prefixes aren't uniform — far more queries start with "s" or "c" than "x" or "z" — so naïve per-letter shards are wildly lopsided. The fix is a **shard-map manager**: analyze the historical distribution and assign *traffic-balanced prefix ranges* (e.g. if "s" alone gets as much traffic as "u–z" combined, give "s" its own shard and group "u–z"), with a lookup table routing each prefix to its shard. Shard by *balanced ranges*, not by letter.
...
- **Rebuild cost and the update gotcha.** Rebuilding a trie of tens of millions of queries is expensive. You *could* update a single node in place, but here's the catch: because every node caches its descendants' top-k, bumping one query's weight forces you to **re-fix every ancestor up to the root** — touching one leaf ripples the whole path. So in practice you rebuild the whole trie (or a changed branch) offline and **publish via atomic swap**, so reads never see a half-built trie and never pay the ripple cost online.
