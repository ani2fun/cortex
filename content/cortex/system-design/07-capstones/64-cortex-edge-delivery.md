...
- **Cloudflare** doesn't meter proxied egress — the economic reason a homelab can put a global CDN in front for €0, the missing piece of [ch 59's](/cortex/system-design/capstones/cortex-capacity-today) egress bind.

---

> **Next:** back to [60. Cortex failure thresholds](/cortex/system-design/capstones/cortex-failure-thresholds) — this chapter lifted the *capacity ceiling*; that one ranks what still *breaks* when you cross the ceilings it can't touch (the single replica and the 1 GiB heap are #1 and #2, and the edge doesn't change that).
