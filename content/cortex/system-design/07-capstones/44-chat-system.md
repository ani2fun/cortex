...
# 44. Chat system (capstone)

## TL;DR
> Real-time chat inverts the feed's trade-offs: instead of eventually-consistent, read-heavy, pull-when-you-open, a chat must deliver **in order**, **in real time** (a long-lived **WebSocket**, not a poll), and **without loss or duplication**, to a device that constantly disconnects. The defining problem is **connection management**: you hold hundreds of millions of persistent connections across a fleet of **gateway** servers (WhatsApp famously packed **~2 million connections per server** on Erlang), and to deliver a message you must find *which gateway holds the recipient right now* — a **session registry** (`user → gateway`, in Redis) plus an **inter-gateway pub/sub** to route the message there. **Ordering** comes from a **per-conversation sequence number** (a local counter, not a costly global clock); **effectively-once** delivery is achieved by **at-least-once delivery + client-message-id dedup**. Offline users get the message **stored and a push notification** (APNs/FCM), delivered on reconnect; a device that's been away **catches up** by asking for everything past the last sequence number it has. The expensive surprise is **presence** ("online", "typing…", "last seen") — naïvely fanning out every status change to every contact is often the real scaling bottleneck. Messages persist in a write-heavy, time-ordered **wide-column store** keyed by `(conversation_id, message_id)` — the same kind of store [Discord runs on Cassandra/ScyllaDB](https://discord.com/blog/how-discord-stores-trillions-of-messages) and Messenger ran on HBase.
...
**Why WebSocket and not polling?** The older way to fake real-time over plain HTTP is **long polling** (Facebook Chat shipped this way for years): the client opens a request and the server *holds it open* until a message arrives or a timeout fires, then the client immediately reopens it. It works, but it's wasteful — every reconnect re-pays the HTTP handshake, the server can't reliably tell when a client has vanished, and (crucially) the reconnecting client may land on a *different* server than the one holding the sender, so you still need cross-server routing. **WebSocket** fixes all three: a single TCP connection [upgraded to full-duplex](https://datatracker.ietf.org/doc/html/rfc6455) (over ports 80/443, so firewalls allow it), held open, with the server free to *push* down it at any moment. **Server-Sent Events (SSE)** is push-only — one-directional — so it can't carry the client's outbound messages, typing events, and acks back up the same pipe. Bidirectional push is exactly what the "double tick" and the live "typing…" indicator need, which is why every modern chat (WhatsApp, Messenger, Slack, Discord) is built on a persistent socket.

## 5. Data model and the central decision

Three stores, each shaped to its access pattern:
- **Message store:** `(conversation_id, seq) → {sender, text, client_msg_id, ts, …}`, where `seq` is a **monotonic per-conversation sequence**. This gives both **ordering** (sort by seq) and **history** (range scan); it's write-heavy and append-mostly → a [wide-column / LSM](/cortex/system-design/storage-and-search/lsm-trees-vs-btrees) store sharded by `conversation_id`. (This is the store the [Discord engineering post](https://discord.com/blog/how-discord-stores-trillions-of-messages) is about — Cassandra, later ScyllaDB; Facebook Messenger ran the same shape on HBase.)
...
**A word on that `seq` — local vs global.** The message ID must be two things at once: **unique** and **time-sortable** (a newer message must always sort after an older one in the same conversation). You could mint IDs from a **global** 64-bit, time-ordered generator — a Snowflake-style scheme (timestamp + machine bits + counter) that's unique across the *entire* system — but that's more machinery than chat needs. The cheaper choice is a **local** sequence: a counter that's only unique *within one conversation*. That's all ordering requires, because **users only ever perceive order inside a single conversation** — nobody can tell whether your message to Alice was globally "before" or "after" some stranger's message to Bob. DDIA makes exactly this point about partitioned logs: within one partition (here, one conversation) messages are totally ordered by a monotonic offset, and *there is deliberately no ordering guarantee across partitions* — which is precisely why per-conversation ordering scales where a global order would bottleneck on a single sequencer.

**Multi-device sync.** A user reads on their phone, their laptop, and the web at once — and all three must converge. The trick is to make the message store the single source of truth and have **each device track its own high-water mark**: a `cur_max_seq` per conversation. On connect (or on a "what's new?" tick) a device asks "give me everything in this conversation with `seq > cur_max_seq`," applies it, and advances its mark. Three devices, three independent cursors, one store — no device has to know about the others, and a message read on the phone simply shows up unread on the laptop until that device catches up.
...
**One step happens before any of this: which gateway do I even connect to?** You can't just round-robin a WebSocket through a load balancer the way you would a stateless REST call, because the connection is *pinned* for its whole lifetime — you want it placed well the first time. So login is a quick **stateless** hop: the client hits an ordinary API server, which asks a **[service-discovery](/cortex/system-design/application-architecture/service-discovery-and-mesh)** layer (Apache Zookeeper is the textbook pick) "what's the best gateway for *this* client right now?" — best by **geography** (terminate close to the user to cut round-trip latency), by **current capacity** (don't pile onto a hot box), and by health. The answer is a specific gateway address; the client then opens its WebSocket straight to it. This cleanly splits the system into a **stateless tier** (login, signup, profile, service discovery — round-robin freely behind a load balancer) and a **stateful tier** (the gateways holding live connections, plus presence), which is the partition every large chat backend settles into.
...
sequenceDiagram
  ...
  participant API as API server (stateless)
  participant SD as Service discovery
  participant GW as Gateway (stateful)
  participant Reg as Session registry
  C->>API: login over HTTPS
  API->>SD: best gateway for this client?
  SD-->>API: gateway address (geo plus capacity)
  API-->>C: connect to that gateway
  C->>GW: open WebSocket, held open
  GW->>Reg: register user to this gateway
  Note over C,Reg: connection now routable, heartbeats keep it alive
```
...
- **Presence is the silent killer.** First, how "online" is even known: a device isn't online because it once connected — it's online because it's still sending a **heartbeat** (say every 5 seconds) and the presence server has seen one within a small window (say the last 30 seconds). The window is the whole point — it rides out the constant micro-disconnects (a phone passing through a tunnel) without flapping a contact's status on and off. The *cost* shows up in fan-out: naïvely, when one user comes online you notify *all* their contacts, typically over a publish/subscribe channel per friend-pair, and "typing…" events fan out continuously — at scale this can dwarf actual message traffic. (A status change in a 100K-member community would emit 100K events.) Mitigate by making presence **lazy / on-demand** (compute it when someone opens a chat or refreshes their friend list, not push it to everyone), batching/debouncing status changes, and isolating the presence path so it can't starve messaging ([bulkheads](/cortex/system-design/distributed-patterns/circuit-breakers-and-bulkheads)).
- **The reconnect storm.** When a gateway (or a whole zone) dies, *all* its millions of connections reconnect **at once** — a thundering herd that can topple the next gateway and the registry. Clients must reconnect with **exponential backoff + jitter** ([Lesson 19](/cortex/system-design/distributed-patterns/idempotency-retries-backoff)); the registry must absorb a re-registration spike.
...
- **Group fan-out — and the two ways to do it.** For a **small** group, the simplest design is **write fan-out**: when someone posts, copy the message into each member's own **inbox** (a per-recipient message sync queue), so every client only ever reads from one place — its own inbox. That's clean precisely *because* the group is small; storing one copy per recipient is fine when there are a few hundred members (WeChat caps groups at 500 for essentially this reason). But the same copy-per-member math is exactly what makes it fall over for a **large** group: a 100,000-member broadcast becomes 100,000 writes per message. So big groups flip to **read fan-out / lazy delivery** — store the message once per conversation and let members pull it when they're active (the [feed celebrity problem](/cortex/system-design/capstones/news-feed) in miniature). Either way it's a [pub/sub fan-out](/cortex/system-design/distributed-patterns/pubsub-and-fanout), and either way the authoritative per-conversation `seq` keeps every member's view ordered identically.
- **The message store write rate.** ~70M messages/s is a colossal write workload; the LSM/wide-column store shards by conversation and is provisioned for write throughput (sequential appends, the LSM sweet spot — [Lesson 24](/cortex/system-design/storage-and-search/lsm-trees-vs-btrees)).
...
| Delivery guarantee | **at-least-once + dedup** vs once-only *transport* | you can't make the *network* deliver a packet exactly once (the ack can always be lost, forcing a resend); but persist + `client_msg_id` dedup makes *processing* idempotent, so the resends are harmless — DDIA calls this **effectively-once**, and it's the right target: never lose a message, never show a duplicate |
...
- **Out-of-order or gaps.** Network reordering or a missed push can leave a client with a gap in `seq`. Because the sequence is **monotonic and gapless within a conversation**, a client that holds 1041 and then receives 1043 *knows* 1042 is missing and requests exactly that range — the same trick a replication follower uses with a log sequence number to resume "without skipping any writes" (DDIA). Ordering is enforced by the client reconciling against `seq`, not by hoping the network behaves. (And note *why* `seq` is a server-assigned counter and not a wall-clock timestamp: clock skew across senders can place a later message *before* an earlier one, silently corrupting order — a logical counter can't.)
- **The same person on three devices.** Phone, laptop, and web must all show the same conversation, and a message you read on one shouldn't stay bold on the others forever. Each device keeps its own `cur_max_seq` per conversation and pulls everything newer on connect (§5); the message store, not the devices, is the source of truth. The subtle bug is treating "delivered to the user" as a single fact — it's per-device, so a read on the phone advances the phone's cursor while the laptop catches up independently.
...
>
> The client attaches a **`client_msg_id`** generated once for that message and **reused on every retry**. The server, before storing, checks whether it has already seen that `(conversation, client_msg_id)`; if so, it returns the **same** `seq` it assigned the first time and does **not** create a second message ([idempotency](/cortex/system-design/distributed-patterns/idempotency-retries-backoff)). So the retry is a no-op and the recipient sees the message once. **Why not lean on "exactly-once" at the transport layer?** Because the *network* can't promise it: after the server commits the message, the ack flowing back can still be lost, so the client — having no way to tell "my message didn't arrive" from "only the ack didn't arrive" — must resend. You can't eliminate the duplicate *delivery*; you can only make the duplicate *harmless*. So you choose **at-least-once delivery** (never lose a message — the unacceptable failure) and pair it with **idempotent processing via the id** (squash the duplicates that at-least-once produces). DDIA's name for the result is **effectively-once**: not "the message crossed the wire exactly once," but "its visible effect is as if it had" — no loss, no duplicate shown. The mistake that breaks it is the client generating a **new** id per attempt — then each retry looks like a fresh message and the dedup has nothing to match on.
>
> </details>
