---
title: '39. Capacity planning and autoscaling'
...
# 39. Capacity planning and autoscaling
...
- **Reactive vs. predictive: react to surprises, schedule the obvious.** There are two ways to call the agency. The **reactive** manager watches the hold queue and calls when it grows — fine for *unpredictable* rushes, but always a step behind because the agents arrive 20 minutes late. The **predictive/scheduled** manager looks at the *calendar*: she knows the lunch rush hits at noon and the holiday-sale lines form Friday, so she books extra agents to be *already seated* before the doors open. Real call centers do both — schedule for the spikes you can see coming, react to the ones you can't. Pure reaction means eating the cold-start gap on every single surge; pure scheduling can't handle a surprise. (This is exactly **target-tracking** vs. **scheduled** autoscaling, which we'll name in §3.)

Agents, Little's Law, the jammed highway, the slow-to-arrive staffing agency, the calendar on the wall, the shared terminal — that's the whole lesson in one building.
...
**Autoscaling** adjusts capacity automatically, and it comes in two flavours you combine rather than choose between. **Reactive** scaling watches a live metric and adjusts after the fact — a feedback loop, always one cold-start behind the demand it's chasing. **Predictive / scheduled** scaling provisions *ahead* of demand you can anticipate: a cron-like rule ("scale to 80 pods at 08:45 every weekday") or a forecast from historical traffic (AWS **Predictive Scaling**, GKE's recommendations). The division of labour is the §2 call-center manager's: schedule for the spikes on the calendar (the 9 a.m. rush, the Friday sale, the Super Bowl ad), react to the ones that aren't.

**Kubernetes HPA** (Horizontal Pod Autoscaler) is the canonical reactive controller. It computes `desiredReplicas = ceil(currentReplicas × currentMetric / targetMetric)` — e.g. 2 pods at 80% CPU with a 50% target → `ceil(2 × 80/50) = 4`. Its defaults encode the "up fast, down slow" rule: **scale-up is immediate** (no stabilization window; may add 100% of pods every 15s), while **scale-down waits a 300-second stabilization window** and takes the *highest* recommendation over that window — and a default **10% tolerance** ignores tiny deviations (the band that stops it flapping on noise). The cloud equivalent is **AWS target-tracking**: you name a target like "70% average CPU" or "1,000 requests per instance" and the Auto Scaling group works backward to the instance count — same `desired = current × metric/target` idea, dressed as a thermostat ("hold this number"). Related tools: **KEDA** (event-driven — scale on queue length, Kafka consumer lag, Postgres row counts, etc., including **scale-to-zero**), **VPA** (vertical — right-sizes a pod's CPU/memory *requests* rather than its count), and the **Cluster Autoscaler / Karpenter** (adds/removes *nodes* when pods can't be placed). That last one stacks a *second* cold start under the first: the HPA asks for pods, but if no node has room a new VM must boot (often **minutes**) before those pods can even start warming up — a node-level lag on top of the pod-level one.

The single most important autoscaling decision is **the signal**: it must be a metric that **falls as you add capacity** so the loop can stabilize — **CPU% per pod**, **requests-per-second per pod**, or **queue depth per worker**. Scaling on *raw request count* (which doesn't drop when you add pods) can never settle, and **CPU is a poor proxy for I/O-bound or async work** (pods blocked on a slow dependency show *low* CPU while requests queue). KEDA exists precisely for those cases: a queue-consumer's true backlog is **messages per worker**, not CPU, so KEDA scales straight off the queue and even drops to zero workers when the queue drains.
...
The flowchart shows *what* the loop does; the state machine below shows *why the timing is asymmetric* — and where the two failure modes (cold-start lag on the way up, flapping on the way down) actually live. Read the right branch as the danger: scale in before the next wave and you snap straight back to `ScalingUp`, re-adding the very pods you just killed.
...
stateDiagram-v2
  [*] --> Steady
  Steady --> Observe: every ~15s, read signal
  Observe --> Steady: within target +/- 10% tolerance
  Observe --> ScalingUp: signal ABOVE target
  Observe --> CoolingDown: signal BELOW target
  ScalingUp --> WarmingUp: add pods now (no wait)
  WarmingUp --> Steady: pods Ready, signal falls
  WarmingUp --> Overload: cold-start lag, spike outruns capacity
  Overload --> WarmingUp: shed load, keep adding
  CoolingDown --> Steady: 300s window still high, KEEP pods
  CoolingDown --> ScaleIn: low for full 300s window
  ScaleIn --> Steady: remove <=10%/min
  ScaleIn --> Thrash: removed too soon, next wave hits
  Thrash --> ScalingUp: re-add the pods just killed
```

<p align="center"><strong>The autoscaler as a state machine. Scaling up risks <code>Overload</code> (new pods aren't <code>Ready</code> yet — the cold-start gap); scaling down risks <code>Thrash</code> (retreat too fast and the next wave snaps you back). The 300s window and 10% tolerance exist to keep you out of those two states.</strong></p>
...
---
flowchart TB
  spike(["sharp spike: 5k to 15k rps in seconds"])
  spike --> known{"spike PREDICTABLE?<br/>(9am rush, flash sale, Black Friday)"}
  known -->|"no, unknown"| react["REACTIVE (HPA / target-tracking)<br/>observe, then add pods"]
  known -->|"yes, known"| pred["PREDICTIVE / SCHEDULED<br/>pre-scale BEFORE the clock"]
  react --> lag["cold-start lag: ~90s to Ready"]
  lag --> gap["GAP: serve 15k on a 5k fleet<br/>~900k requests dropped"]
  gap --> shed["mitigate: pre-warm headroom<br/>+ shed load while growing"]
  pred --> ready["capacity already warm at T-0<br/>no cliff"]
  shed --> ok(["absorbs the shock"])
  ready --> ok
```

<p align="center"><strong>Two paths through the same spike. Reactive scaling always pays the cold-start gap (left); predictive/scheduled scaling pre-warms before a spike it can anticipate (right). The fix for a <em>known</em> spike is to not be reactive about it.</strong></p>

**Fix:** **predictive/scheduled** scaling for *known* spikes (pre-scale before the 9 a.m. rush or the sale), keep pre-warmed headroom, shorten startup, and **shed load gracefully** while the fleet grows rather than collapsing.

**Finding the per-pod number honestly — load test to the cliff.** Every figure above hangs on one input: "50 concurrent per pod," "100 rps/pod." Guess it and the whole plan is fiction. The way to *know* is a **load test** that ramps a single pod until it breaks: throughput climbs with offered load, plateaus, then **p99 latency hooks upward** while throughput flattens or falls — that knee is the **cliff** (the `ρ → 1` curve made empirical). The safe per-pod target sits at ~70% of the *plateau*, not the breaking point — the headroom is the gap between the knee and where you operate. Run it again after every release: a slow query or a fatter dependency moves the cliff, and a capacity plan calibrated to last quarter's binary silently over- or under-provisions today's.
...
- **Autoscaling into a non-scalable dependency.** Adding app pods when the bottleneck is the database (connection limit, slow query) just points more load at the real constraint and **amplifies** the outage ([Lessons 23](/cortex/system-design/distributed-patterns/circuit-breakers-and-bulkheads)/[34](/cortex/system-design/application-architecture/service-discovery-and-mesh)). Cap `maxReplicas`, scale or protect the dependency (pooler, replica), and shed load.
...
- **A stale per-pod number.** Every estimate rests on "what one instance handles," and that number drifts: a slow query, a heavier dependency, or a new release moves the cliff. A plan calibrated to last quarter's binary silently over- or under-provisions today's. **Re-load-test to the knee after meaningful releases** and recheck the HPA target against it.
