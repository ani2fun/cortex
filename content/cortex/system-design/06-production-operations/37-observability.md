---
title: '37. Observability — logs, metrics, traces'
...
Two complementary frames decide *which* metrics a dashboard should carry, and they split cleanly by what you're watching. **RED** — **R**ate, **E**rrors, **D**uration — is for **request-driven services** (an API, a web tier): how many requests per second, what fraction fail, and how long they take. It's the caller's-eye view, and it maps almost one-to-one onto the golden signals (rate = traffic, errors = errors, duration = latency). **USE** — **U**tilization, **S**aturation, **E**rrors — is for **resources** (a CPU, a disk, a connection pool, a Kafka partition): how busy it is, how much work is queued waiting for it, and its error count. The pairing is the senior instinct: when RED says a service is slow (high duration), USE on its resources usually tells you *why* (the database connection pool is saturated, the disk is at 100% utilization). RED is the symptom; USE is the cause — which is exactly why you alert on RED/golden signals (§6) and keep USE on a dashboard for the follow-up drill-down.

### The telemetry pipeline

All three pillars flow through the same four-stage pipeline, and naming the stages makes the tooling fall into place. You **instrument** the app (emit the data), **collect** it through an agent that batches and routes, **store** each signal in a backend built for *its* shape, and **consume** it via dashboards and alerts. The classic system-design framing breaks the same pipeline into five components — collection, transmission, storage, alerting, and visualization — but it's the same flow.
...
---
flowchart LR
  subgraph APP["instrument · in your app"]
    code["OpenTelemetry SDK<br/>emits metrics · logs · spans"]
  end
  subgraph COLL["collect · agent"]
    otelc["OTel Collector<br/>batch · sample · enrich · route"]
  end
  subgraph STORE["store · purpose-built backends"]
    tsdb[("metrics<br/>TSDB — Prometheus")]
    logs[("logs<br/>Loki / Elastic")]
    traces[("traces<br/>Jaeger / Tempo")]
  end
  subgraph USE["query · consume"]
    dash["dashboards<br/>Grafana"]
    alert["alerting<br/>SLO burn · golden signals"]
  end
  code -->|"OTLP push, or /metrics pull"| otelc
  otelc --> tsdb
  otelc --> logs
  otelc --> traces
  tsdb --> dash
  ...
  traces --> dash
  tsdb --> alert
  alert -->|"page on symptom"| pager["on-call human"]
```

<p align="center"><strong>The telemetry pipeline. Instrument once (OpenTelemetry), collect through an agent that batches/samples/routes, fan out to a backend purpose-built for each pillar's shape, and consume via dashboards (Grafana) and SLO-based alerts. Each store is specialized: a metrics TSDB ([Lesson 26](/cortex/system-design/storage-and-search/time-series-databases)) is wrong for full-text logs, and a log store is wrong for span trees.</strong></p>

Two details in that pipeline carry real weight. **Collection is pull or push**, and the choice has consequences. In the **pull** model (Prometheus's default) the collector periodically scrapes a `/metrics` HTTP endpoint each service exposes — which gives you a free liveness check (a target that fails the scrape is visibly down) and lets you curl the endpoint from your laptop, but the collector must *discover* every target (via the service discovery of [Lesson 34](/cortex/system-design/application-architecture/service-discovery-and-mesh)). In the **push** model (CloudWatch, Graphite, StatsD) an agent on each host ships metrics outward — which suits short-lived jobs and serverless functions that vanish before any scrape could reach them, but gives you no built-in "is it up?" signal. Most large shops run both. The second detail: the **collector** (e.g. the OpenTelemetry Collector) is the pipeline's pressure-relief valve — it batches, re-samples, strips secrets, and can buffer to a queue like Kafka so a momentary storage outage doesn't drop telemetry on the floor.
...
Checkout starts feeling slow. Watch the three pillars do their distinct jobs — and notice that the **same `trace_id`** is what lets you hand off from one pillar to the next.
...
---
flowchart TB
  req["one request · trace_id=abc123"]
  req --> M["METRICS<br/>checkout.latency p99 ↑<br/><i>is something wrong?</i>"]
  req --> T["TRACES<br/>span tree, abc123<br/><i>where did it break?</i>"]
  req --> L["LOGS<br/>level=error trace_id=abc123<br/><i>what exactly happened?</i>"]
  M -.->|"alert fires → open the trace"| T
  T -.->|"filter logs by trace_id"| L
```

<p align="center"><strong>One request emits into all three pillars, and the propagated <code>trace_id</code> is the join key. Metrics raise the alarm; you pivot to the trace for the same request; you filter logs by that trace_id for the exact error. Without the shared ID these are three disconnected haystacks — <em>correlation</em> is what turns three tools into one investigation.</strong></p>
...
The third pillar deserves its own line, because *how* you log decides whether logs are searchable. **Structured logging** means emitting logs as machine-parseable key-value records (JSON), not free-text prose — and crucially, stamping each line with the **`trace_id`** so a log filter jumps straight to the §4 trace:
...
def log_event(level, msg, **fields):
    ...
    record = {"level": level, "msg": msg,
              "trace_id": format(span.trace_id, "032x"),  # ← the join key from the §4 diagram
              **fields}                                    # structured: queryable, not grep-bait
    logging.info(json.dumps(record))

# log_event("error", "charge failed", tenant="acme", amount=4200, gateway="stripe")
# → {"level":"error","msg":"charge failed","trace_id":"abc123...","tenant":"acme",...}
```

The payoff is that `level=error AND tenant=acme` becomes a real query instead of a regex over text — and `trace_id=abc123` ties the log line back to its trace and metrics. (The same rule from §4 applies: a free-text line like `"user 12345 failed"` smuggles high cardinality into an *unindexed* blob; put the ID in a structured field instead.)

**Concretely, a common open-source stack:** Prometheus scrapes and stores the metrics (a purpose-built TSDB, [Lesson 26](/cortex/system-design/storage-and-search/time-series-databases)); **Grafana** renders the dashboards and fires alerts on PromQL; **Jaeger** (or Zipkin — both direct descendants of Dapper) stores and visualizes the trace waterfalls; and a log store (Loki, Elasticsearch) holds the structured logs — all fed by the OpenTelemetry Collector from the §3 pipeline. And the alert you build on that SLI should be a **burn-rate alert**, not a raw threshold. A naive `latency > 300ms` threshold pages on every transient blip (alert fatigue). A burn-rate alert instead asks *"are we spending the error budget too fast?"* — e.g. page only if we're burning budget fast enough to exhaust the whole 30-day allowance in a few hours. That ties the page directly to user harm (the SLO), which is the §6 discipline of alerting on symptoms, not causes.
...
- **Cardinality bomb in metrics.** High-cardinality labels (`user_id`, `request_id`, URLs) multiply time-series and OOM the metrics backend ([Lesson 26](/cortex/system-design/storage-and-search/time-series-databases)). Keep metric labels bounded; put per-request detail on **traces/logs**, which are designed for it.
